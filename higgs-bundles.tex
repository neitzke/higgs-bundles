...
Let us describe its symplectic and \kahler structures.
At any $x \in \Jac(C)$ we have $T_x \Jac(C) = H^1(C,\R)$,
and the symplectic form $\omega$ is just given 
by the intersection pairing
\begin{equation}
  \omega(\alpha, \beta) = \int_C \alpha \wedge \beta.
\end{equation}
In particular,
as a symplectic manifold, $\Jac(C)$ does not depend on
the complex structure of $C$. On the other hand the
...
\begin{exercise}
Prove that the complex and symplectic structures on $\Jac(C)$
induced by the \kahler quotient construction are the 
ones claimed above.
\end{exercise}
