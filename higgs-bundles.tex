...
is a subsheaf of the locally free sheaf of sections of $E'$, thus
...
consisting of all sections vanishing at a point, which is isomorphic
to the sheaf of sections of $\cO$, but its saturation is the whole $\cO(1)$.)
...
Suppose that $\cO$ contains a minimum of $J$, corresponding
to a unitary connection; abusing notation we call this connection $D$.
We will show that this $D$ has the desired property $F_D = 0$.
...
The leading change of $\star F_D$ is $\I t D^* D Z$
(because $D$ changes by $t \bar\partial_D Z - t \partial_D Z$,
so $F_D$ changes by $t (\partial_D \bar\partial_D - \bar\partial_D \partial_D) Z = \star \I t D^* D Z$). \fixme{check sign}
...
reduces $J(D)$, unless already $F_D = 0$.
Thus, if $D$ is a minimum of $J$, then it must have $F_D = 0$
as desired.

Thus the problem remaining is to show that $J$ has a minimum on $\cO$.
Let $m$ be the infimum of $J$ on $\cO$, and
consider a sequence $D_n$ with
$J(D_n) \to m$.
...
$J(D_\infty) \le m$. \fixme{explain why} Thus, if
...
structure even though it's only $L^2_1$: from Atiyah-Bott Lemma 14.8} There
...
some proper holomorphic subbundle $F$ of $(E,\bar\partial_{D_\infty})$; 
...
$\bar\partial_D$ and on the bottom $\bar\partial_{D_\infty}$;
...
In particular, this means $(E,\bar\partial_{D_\infty})$ (the bottom row) is
\ti{unstable}. (That is as we should expect: $D_\infty$ is
...
connections in the orbit of $(E,\bar\partial_{D_\infty})$,
as follows.
...
that $D_\infty$ was the limit of an infimizing sequence.
