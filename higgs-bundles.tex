...
It follows that $X \kq G = Y / G$ is also a manifold, of the desired 
dimension. The tangent space $T(X \kq G) = T(Y/G) = TY / \rho(\fg)$.
\fixme{give reference for this; note it's 
important that $G$ is compact, else have counterexamples 
like $\R$ acting on the torus by an irrational translation}
...
$T(X \kq G)$ is
\begin{equation}
  T(X \kq G) = \left(\{ \grad \mu_Z: Z \in \fg \} \oplus \{ \rho(Z): Z \in \fg \}\right)^\perp \subset TX.
\end{equation}
But 
\begin{equation}
...
\begin{equation} \label{eq:ooguri-vafa-V-approx}
  V \approx V^\sf = -\frac{1}{2\pi} \log \abs{z} + C' + C
\end{equation}
...
the \hk structure. (In fact, there is a subtle choice here:
...
only if $\zeta = z / \abs{z}$. (Hint: to see that the disc \ti{does}
...
To see that $\cX_2$ is indeed a holomorphic function on $X$, 
...
\end{equation}
(this computation can be found in \cite{Gaiotto:2008cd}.)

One tricky aspect of this story is that the function $\cX_2$ is 
...
One can however compute the \ti{limits} from the two sides, 
\begin{equation}
  \cX_2^\pm = \lim_{\eps \to 0^\pm} \cX_2(\zeta \e^{\I \eps}),
\end{equation}
...
Then the two limits differ by the integral around the pole
at $\zeta = \zeta'$, which can be evaluated by the residue theorem,
giving \fixme{check sign}
\begin{align}
  \cX_2^+(\zeta) &= \cX_2^-(\zeta)(1 - \cX_1(\zeta)) \text{ for } \zeta \in \ell_+, \label{eq:ooguri-vafa-cluster-1} \\
  \cX_2^+(\zeta) &= \cX_2^-(\zeta)(1 - \cX_1(\zeta)^{-1})^{-1} \text{ for } \zeta \in \ell_-. \label{eq:ooguri-vafa-cluster-2}
\end{align}
...
\fixme{ref Beauville?} \fixme{need $X$ simply connected? it's part of the usual statement, but doesn't seem to be used in the proof!}
Suppose that:
\begin{itemize}
  \item $X$ is a compact complex manifold,
...
for skew-symmetric matrices, it follows that $I_2$ is
a direct sum of blocks like $\begin{pmatrix} 0 & c_i \\ -c_i & 0 \end{pmatrix}$.
...
by $I_2$. We have a similar block decomposition for the
...
of $TX$ which acts by a different constant on each $E_i$.
...
the holomorphic $1$-form $\de x / y$. We want to wedge this with
...
and $B(u)$ a section of $\cO(12)$. With $A$ and $B$ chosen,
...
We remark that the 
twistor space of $X \hkq G$ can be obtained from 
$\cZ$ by fiberwise holomorphic symplectic quotient.
...
A very nice reference on this material is \fixme{Thaddeus}.
...
\begin{equation}
  \deg E = c_1(E) \cdot C.
\end{equation}
\fixme{should define this in some more primitive way}
\end{defn}
...
We fix a single rank $K$ complex vector bundle $E$, of degree $d$.
(All such bundles are equivalent; just fix one.)

The space we want to describe is, from one point of view,
the space of all $\bar\partial$ operators on $E$, modulo
equivalence. Here ``equivalence'' means the (infinite-dimensional,
complex) group $\fG_\C$ of sections of $GL(E)$.

The neat trick, explained in \cite{MR85k:14006}, 
is to think of this as a \kahler
quotient in an infinite-dimensional setting. 
Namely we first fix a Hermitian metric on $E$.
After so doing, we have an infinite-dimensional real group
$\fG$ of \ti{unitary} equivalences, i.e. sections of $U(E)$.

Now we look at the space $\cA$ of \ti{all} unitary 
connections on $E$.
This is an infinite-dimensional affine space, modeled
on the linear space $\Omega^{1}(\End E)$.
On the latter space we have a natural symplectic form
\begin{equation}
  \omega(\alpha,\beta) = \int_C \Tr(\alpha \wedge \beta).
\end{equation}
Moreover the group $\cG$ acts on $\cA$, preserving
$\omega$, with moment map
\fixme{...}
