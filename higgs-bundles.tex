...
It is possible to add a single point over $x_i$
...
\fixme{explain this more precisely}
\end{example}
...
% structure. 
To make this precise, first observe the following:

\begin{prop} Suppose $X$ is a \hk manifold. For each $\vec s \in S^2$,
let $I = I_{\vec s}$, and let
\begin{equation}
 L_{\vec s} \subset \Omega^2_\C(X)
\end{equation}
be the vector space spanned by $2$-forms
$\omega_{I'} + \I \omega_{II'}$, where $I' = I_{s'}$ and
$s' \perp s$. Then:
\begin{enumerate}
\item $L_{\vec s}$ is a $1$-dimensional complex subspace. 
\item Any $\Omega \in L_{\vec s}$ is a holomorphic symplectic form
on $(X,I_{\vec s})$.
\item If we equip $S^2$ with the opposite of its standard 
...
\begin{pf} Here is a direct computational proof.
...
\end{equation}
where $I_{S^2}$ denotes the opposite of the 
canonical complex structure on $S^2$.
\end{defn}
