...
by taking the orthocomplement, i.e. imposing the gauge-fixing condition
$D^* \dot{A} + \star [\dot\Phi, \Phi] = 0$.
So altogether the tangent space we are after is the kernel of
the operator
\begin{equation}
(\dot A, \dot \Phi) \mapsto \begin{pmatrix} D \dot{A} + [\Phi,\dot\Phi] \\ D \dot\Phi + [\Phi, \dot A] \\ D \star \dot\Phi + [\star \Phi, \dot A] \\ D \star \dot{A} + [\Phi, \dot \Phi] \end{pmatrix}
\end{equation}
Note that this operator is \ti{elliptic}, just as before:
the principal symbol is $2K^2$ copies of $\de \oplus \de^*$.
Thus, as before, elliptic theory tells us that when we 
consider this operator on the Banach spaces of $L^2_k$ 
Higgs pairs, we will find it has a finite-dimensional
