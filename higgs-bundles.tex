...
that $D = \de + A$,
where
\begin{equation} \label{eq:chern-connection-local}
  A = h^{-1} \partial h.
\end{equation}
...
\begin{defn}[Laplace operator coupled to a vector bundle]
If $X$ is a Riemannian manifold and $E$ a Hermitian vector bundle over 
$X$ with connection $D$, we define a Laplacian
\begin{equation}
...
\begin{equation}
  g(\alpha,\beta) = - \int_C \Tr(\alpha \wedge \star \beta).
\end{equation}
\end{exercise}
...
\begin{equation} \label{eq:symplectic-form-jac}
  \omega(\alpha, \beta) = \int_C \alpha \wedge \beta.
\end{equation}
...
$x_i$, $y_i$ we introduced above, \fixme{sign?}
...
\ti{complex} structure definitely does depend on the complex
...
\begin{equation}
  g(\alpha, \beta) = \int \alpha \wedge \star \beta
\end{equation}
where $\alpha$, $\beta \in \cH^1(C)$.
...
Extend \autoref{exc:connections-vs-representations} to general 
degrees $d$: in other words,
give a description of $\cN_{K,d}^s(C)$ and $\cN_{K,d}(C)$
as sets of equivalence classes of 
representations of an \ti{extension} of $\pi_1(C)$
by a finite group. (Hint: use the description of $C$ as 
a polygon with edges identified; what can you say about the
holonomy around the boundary of the polygon?)
\end{exercise}

\begin{exercise} \label{exc:no-irreducible-connections-on-torus} 
Show that, when $g=1$ ($C$ is a torus), 
and $K>1$, $\cN_{K,d}^s(C)$ is empty. (Hint: use the result
of \autoref{exc:connections-vs-representations-general}.)
\end{exercise}
...
Then we have, using \eqref{eq:chern-connection-local},
\begin{equation}
...
such that this sequence becomes \ti{convergent}
...
\end{equation}
This uses the fact that holomorphically $(E,\bar\partial_D) \simeq (E,\bar\partial_{D_n})$ --- in other words, the operator $D^*_n \otimes D$
...
for all $D_n$ then it is also zero for $D_*$),
so $D_*^* \otimes D$ also has a nontrivial kernel,
...
and $\det \rho$ is generically nonzero (this comes from 
the maximality of $K$ and minimality of $F$.)
...
in the closure of the $\fG_\C$-orbit of $D$, so if it
...
\begin{defn}[Stable Higgs bundle] A Higgs bundle $E$ over $C$ is called:
\begin{itemize}
...
\begin{example}[Hitchin section for $K=2$] \label{exa:hitchin-section-K2} 
...
\begin{equation}
  E = \cL \oplus \cL^{-1}
\end{equation}
For any $(\phi_1, \phi_2) \in H^0(C,K_C) \oplus H^0(C,K_C^2)$,
we can equip $E$ with the Higgs field
\begin{equation}
  \varphi = \begin{pmatrix} \phi_1 & \phi_2 \\ 1 & \phi_1 \end{pmatrix} \in \begin{pmatrix} \Hom(\cL,\cL) \otimes K_C & \Hom(\cL^{-1},\cL) \otimes K_C \\ \Hom(\cL,\cL^{-1}) \otimes K_C & \Hom(\cL^{-1},\cL^{-1}) \otimes K_C \end{pmatrix} = \begin{pmatrix} K_C & K_C^2 \\ \cO & K_C \end{pmatrix}
\end{equation}
...
bundles modulo equivalence, and we want it to be somehow ``nice.''
The simplest idea would be just to take all stable vector bundles
and then equip them with arbitrary Higgs fields: in other words
take a vector bundle over 
$\cN_{K,d}(C)$, with fiber $H^0(C, \End E \otimes K_C)$.
The holomorphic tangent space to $\cN_{K,d}(C)$ at $E$ is the space
$\cH^{0,1}(\End E)$ of harmonic forms $\alpha \in \Omega^{0,1}(\End E)$.
This space is naturally dual to $H^0(C, \End E \otimes K_C)$, via the 
pairing
\begin{equation}
  (\alpha, \psi) \in \cH^{0,1}(\End E) \times H^0(C,\End E \otimes K_C) \mapsto \int_C \Tr (\alpha \wedge \psi).
\end{equation}
Thus the moduli space we obtain in this way would be $T^* \cN_{K,d}(C)$.
In particular this is a \ti{holomorphic symplectic} manifold.
But it is not quite the space we are after.

To get the space we are really after, we proceed by a ``doubled''
version of the infinite-dimensional \kahler quotient we took before,
i.e. we take an infinite-dimensional
\hk quotient. As before, we fix a smooth bundle $E$ over $C$, 
of degree $d$.
Now we define a ``doubled'' version of the space of $\bar\partial$-operators
on $E$,
\begin{equation}
...
We denote points of $\cA^\C_1$ as
\begin{equation}
  (\bar\partial_E, \varphi) \in \cA^\C_1.
\end{equation}
Evidently we have
\begin{equation}
...
\begin{equation} \label{eq:Omega-1-AC}
  \Omega_1((\alpha_1, \psi_1),(\alpha_2, \psi_2)) = \int \Tr (\alpha_1 \wedge \psi_2 - \alpha_2 \wedge \psi_1).
\end{equation}

\begin{exercise}
Verify that the holomorphic symplectic form \eqref{eq:Omega-1-AC} 
is the natural one on $T^* \cA^{\bar\partial}$.
\end{exercise}

As for $\cA^{\bar\partial}$, the space $\cA^\C_1$ has a natural action
of $\fG_\C$, by
\begin{equation}
  (\bar\partial_E,\varphi) \mapsto (\bar\partial_E^g = g \bar\partial_E g^{-1}, \varphi^g = g \varphi g^{-1}).
\end{equation}

\begin{prop}[Moment map for $\fG_\C$-action on $\cA^\C_1$]
The $\fG_\C$-action on $\cA^\C_1$ has a complex moment map
\begin{equation}
  M_1 = \bar\partial_E \varphi.
\end{equation}
More precisely, this means: given $Z \in \Lie \fG = \Omega^0(\End E)$
we have
\begin{equation}
  \mu_Z(\bar\partial_E,\varphi) = \int_C \Tr(Z \bar\partial_E \varphi).
\end{equation}
\end{prop}

\begin{pf} This follows the pattern of proof of \autoref{prop:unitary-connections-moment-map}.
For $Z \in \Lie \fG_\C = \Omega^0(\End E)$ we have
the infinitesimal action
\begin{equation}
  \rho(Z) (\bar\partial_E,\varphi) = (- \bar\partial_E Z, [\varphi,Z]),
\end{equation}
and for
$(\alpha,\psi) \in \Omega^{0,1}(\End E) \times \Omega^{1,0}(\End E) = T\cA_1^\C$
we have
\begin{equation}
  \de \mu_Z (\alpha,\psi) = \int_C \Tr( Z [\alpha,\varphi] + Z \bar\partial_E \psi)
\end{equation}
We compute:
\begin{align}
  \Omega_1(\rho(Z), (\alpha,\psi)) &= \int_C \Tr(- \bar\partial_E Z \wedge \psi - \alpha \wedge [\varphi,Z]) \\
  &= \int_C \Tr(Z \bar\partial_E \psi + Z[\alpha,\varphi]) \\
  &= \de \mu_Z(\alpha,\psi)
\end{align}
as desired.
\end{pf}

Thus formally the moduli space of Higgs bundles would appear to be
\begin{equation}
  M_1^{-1}(0) / \fG_\C = \cA^\C_1 \kq \fG_\C.
\end{equation}
As we have noted, though, the holomorphic symplectic quotient of 
an \hk space has another interpretation: we can sometimes interpret
it as an \hk quotient by a smaller group. 
...
\begin{align}
    ...
    (\bar\partial_E,\varphi) & \mapsto (D_h, \Phi = \varphi - \varphi^\dagger)
\end{align}
where as usual $D_h$ denotes the Chern connection corresponding
to $\bar\partial_E$.

$\cA^H$ also carries a natural real symplectic form
\begin{equation}
  \omega_1((\dot A_1, \dot \Phi_1),(\dot A_2, \dot \Phi_2)) = \int \Tr(\dot A_1 \wedge \dot A_2 + \dot \Phi_1 \wedge \dot \Phi_2),
\end{equation}
along with the complex form $\Omega_1$ which we break into 
components $\Omega_1 = \omega_2 + \I \omega_3$.

\begin{prop}[\Hk structure on $\cA^H$] The forms
$(\omega_1, \omega_2, \omega_3)$ on $\cA^H$ are the symplectic
forms for a \hk structure on $\cA^H$, with \hk metric
\begin{equation}
  g = \fixme{...}
\end{equation}
\end{prop}

\fixme{describe the 3 complex structures and who's holomorphic in each}
\fixme{structure $I_1$: $A_z$, $\Phi_z$}
\fixme{structure $I_2$: ...}
\fixme{structure $I_3$: ...}
