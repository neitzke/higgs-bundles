...
to be very close to the true \hk metric ``near the ends'' of $\cM$, i.e. 
when we go out along a generic path to $\infty$.
...
\ti{complexify} the above formulas.
\begin{prop}[Complex twistor lines for $\cM_{K,d}(C)$]
\fixme{roughly!} The space of \ti{complex} holomorphic 
sections of the twistor space $\cZ \to \C\PP^1$
contains the set of tuples $(D,\varphi,\bar\varphi)$,
where $D$ is a $GL(K,\C)$-connection over $C$,
$\varphi \in \Omega^{1,0}(\End E)$, $\bar\varphi \in \Omega^{0,1}(\End \overline E)$, obeying
\begin{subequations}
\begin{align}
\bar\partial_E \varphi &= 0, \\
\partial_E \bar\varphi &= 0, \\
F_D + [\varphi,\bar\varphi] &= 0.
\end{align}
\end{subequations} 
\end{prop}
\begin{pf} Given such a tuple we can write the usual formula
$$ \nabla(\zeta) = \zeta^{-1} \varphi + D + \zeta \bar\varphi $$
which gives the desired section. \fixme{explain why it extends
over the poles}
\end{pf}
...
of \eqref{eq:X-sf}. Along the way we will discover various
extra interesting bits of structure.

We need one technical bit of preparation:
...
% (So $\varepsilon$ measures the extent to which $\sigma$ fails to be a
% homomorphism.)

The key lemma we will use is:
\begin{lem} Suppose given a lattice $\Gamma$ with: 
\begin{itemize}
...
\begin{equation}
  T = \Hom(\Gamma, \C^\times).
\end{equation}
Note that $T$ is an abelian group.
Define
$$ \cX^\sf: \C^\times \times T \to T $$
as follows: $\cX^\sf(\zeta): T \to T$ is multiplication by
the element $\exp(Z / \zeta + \bar{Z} \zeta) \in T$.
Then there exists at most one map
$$ \cX: \C^\times \times T \to T $$
with the properties:
\begin{itemize}
\item 
As $\zeta \to 0$ or $\zeta \to \infty$, the composition $\cX^\sf(\zeta) \circ \cX(\zeta)^{-1}$
has a finite limit.
\item $\cX(\zeta)$ is analytic in $\zeta$ away from the countable collection of rays 
labeled by primitive vectors $\mu \in \Gamma$,
$$\ell_\mu = \{ \zeta: Z_\mu / \zeta \in \R_- \} \subset \C^\times $$
while on each of these rays $\cX$ jumps by the transformation
\fixme{be precise about clockwise/counterclockwise}
\begin{equation}
  \cX \to \cX \exp \left[ (n\mu)^* \sum_{n \ge 1} DT(n\mu) \log(1 - \sigma(n\mu) \cX_{n\mu}) \right].
\end{equation}
or more concretely,
\begin{equation}
  \cX_\gamma \to \cX_\gamma \prod_{n \ge 1} (1 - \sigma(n\mu) \cX_{n\mu})^{n \mu^*(\gamma) DT(n\mu)}
\end{equation}
\end{itemize}
\end{lem}

\begin{pf} Suppose $\cX$ and $\cX'$ obey all of these conditions. \fixme{..} 
\end{pf}




\fixme{trajectories of quadratic differentials and triangulations}
