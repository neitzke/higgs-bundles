...
In fact, for our purposes we can basically specialize to the operator
\begin{equation}
...
The (principal) 
\ti{symbol} of an order $k$ differential operator $D$ mapping 
...
\begin{equation}
  \sum_{i_1, \dots, i_k} {P_{i_1 \cdots i_k}} \partial_{i_1} \cdots \partial_{i_k}
\end{equation}
...
\begin{equation}
  \sigma = \I^k \sum_{i_1, \dots, i_k} {P_{i_1 \cdots i_k}} p_{i_1} \cdots p_{i_k}
\end{equation}
\end{defn}

\begin{exercise} Verify that the definition \autoref{def:symbol}
is well formulated, i.e. that the symbol $\sigma$ so obtained
does not depend on the coordinate system we choose.
\end{exercise}

\begin{defn}[Elliptic differential operator] \fixme{...}
\end{defn}
