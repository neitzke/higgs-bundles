...
point $u \in \cB$, we get a homomorphism $Z(u): \Gamma_{\vec \phi} \to \C$,
...
\begin{equation}
  X = \Hom(\Gamma, U(1)),
\end{equation}
i.e. the fiber of $X$ over $u \in \cB$ is $\Hom(\Gamma_{\vec \phi}, U(1))$,
...
\end{prop}
\begin{proof} A proof for $K=2$ can be found in \cite{MR89a:32021}.
...
Then we have 
\begin{align}
 \de f_\alpha(\dot A, \dot \Phi) &= \int_C \alpha \Tr(\dot\varphi \varphi^{n-1}) = 2 \I \int_C \Tr(\dot\varphi \dot A_{\bar z}) = 2 \I \int_C \Tr\left(\dot\varphi \cdot -\frac{\I}{2} \alpha \varphi^{n-1} \right) \\
...
$\alpha$'s, the $f_\alpha$ give a linear coordinate system on
...
Our next aim is to describe the \hk metric on $\cM_{K,d}(C)$
in a concrete way.
For this we first have to specify what $\cM_{K,d}(C)$
is as a manifold.
...
\begin{prop}[``Griffiths transversality'' for spectral curves]
...
\begin{proof} A tangent vector $v$ to $\cB$
...
in $\Gamma^*_\C$ by integration. Thus we get
...
tori but not
canonically trivialized, because of the 
...
we want to erect a coordinate system. We know how to deal with this problem
in the important special case $d = -K(g-1) = \half \chi(C)$. 
Said otherwise, this is the case where $E \otimes K_C^{1/2}$ 
has degree $0$. 
In this case we get $d' = -K^2(g-1) = \half \chi(\Sigma)$,
so $\cL \otimes K_\Sigma^{1/2}$ likewise has degree $0$. 
\fixme{twisted characters, coordinates $\theta_\gamma$}

% when $d=0$. Namely we
% work locally: fix a basepoint $\vec\phi \in \cB$
% and a point $p \in \Sigma_{\vec\phi}$. Then $p$ extends canonically to a local section 
% of the bundle of curves $\Sigma_{\vec\phi}$, over any contractible patch $U \in \cB'$. 
% We use the holomorphic line bundle
% $\cO(d' p)$ as a basepoint in the torus fibers over $U$. 
% Having done so, for any $\vec\phi \in U$ we have an identification
% $$ \rho^{-1}(\vec\phi) \simeq \Hom(\Gamma, U(1)) $$
% and thus functions
% $$ \theta_\gamma: \rho^{-1}(U) \to \R / 2 \pi \Z. $$
% These are angular coordinates on the torus fibers.

% Fortunately, the dependence on the choice of $p$ is mild:

% \begin{exercise} Verify that changing the choice of the point $p$
% changes the coordinate functions $\theta_\gamma$ by
% \begin{equation}
%   \theta'_\gamma = \theta_\gamma + 2 \pi d' \int^{p'}_p \alpha_\gamma
% \end{equation}
...
\begin{equation}
	\cX_\gamma: \rho^{-1}(U) \times \C^\times \to \C^\times
\end{equation}
by
\begin{equation}
	\cX_\gamma(\zeta) = \exp \left(\zeta^{-1} Z_\gamma + \I \theta_\gamma + \zeta \bar{Z}_\gamma \right)
\end{equation}
and a complex $2$-form
\begin{equation}
	  \Omega(\zeta) = \I \DIP{\de \log \cX(\zeta), \de \log \cX(\zeta)}.
\end{equation}
...
and we have introduced an additional global ``twisting'' from \fixme{...}
