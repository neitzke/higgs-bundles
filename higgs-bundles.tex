...
smoothness etc.
Moreover, suppose that $\cO$ contains a minimum of $J$, corresponding
...
For this purpose consider a sequence $D_n$ along which
$J(D_n)$ approaches its infimum.
Using the Uhlenbeck compactness
theorem \autoref{thm:uhlenbeck-weak} we can make gauge transformations
such that some subsequence becomes convergent
(in the weak $L^2_1$ sense), to
some limiting connection $D_\infty$. If
$D_\infty \in \cO$ then we are done.
...
holomorphically $(E,\bar\partial_D) \simeq (E,\bar\partial_{D_n})$,
which implies the operator
