...
\begin{exercise} Suppose $X$ is a \kahler manifold, with a function $\mu$
...
\begin{exercise} Compute the function $\mu$ on $\cM_{K,d}(C)$
...
we want to erect a coordinate system. To deal with this problem we
...
and thus functions
$$ \theta_\gamma: \rho^{-1}(U) \to U(1). $$
These are angular coordinates on the torus fibers.

\begin{exercise} How do the coordinate functions $\theta_\gamma$
