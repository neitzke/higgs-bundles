...
and the discriminant $\Delta_{\vec\phi}$ is a section of 
$K_C^{K(K-1)}$.
\end{pf}
...
of the singular spectral curves. The most interesting fiber is the
one over $0 \in \cB$: indeed the whole $\cM^s_{K,d}(C)$ 
deformation retracts to this fiber, 
the ``nilpotent cone,'' via the gradient flow of a rather interesting
function
\begin{equation}
  F(D,\Phi) = \int_C \Tr (\varphi \wedge \varphi^\dagger).
\end{equation}

\begin{exercise} Compute the function $F$ on $\cM_{K,d}(C)$
...
The fibers of the Hitchin fibration have dimension equal to that
...
\begin{equation}
  \Omega_1^{-1} (\de f_\alpha) = (\dot{A}_{\bar z} = -\frac{\I}{2} \alpha \varphi^{n-1}, \dot\varphi = 0)
\end{equation}
...
Finally note that the $v_\alpha$
\ti{span} the tangent spaces to the fiber.
For this note that the $\de f_\alpha$ span $T^* \cB$: indeed
we may choose $\alpha$'s corresponding to a basis of
$[H^0(C, K^n)]^*$.

Thus we conclude that $\Omega_1 = 0$ when restricted to the fiber, as 
desired.
...
\fixme{$Z$, $\Gamma$, $\cX$ etc for this example}
