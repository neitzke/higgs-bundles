...
If $X$ is \hk then
\begin{equation}
I_1 = \omega_2^{-1} \omega_3
\end{equation}
and cyclic permutations.
(What this formula really means: view $\omega_3$ as a map $TX \to T^* X$
namely $v \mapsto \omega_3(v,\cdot)$,
and $\omega_2^{-1}$ as a map $T^* X \to TX$
namely $\omega_2(v,\cdot) \mapsto v$; then the composition
$\omega_2^{-1} \omega_3: TX \to TX$ is $I_1$.)
\end{prop}
\begin{pf}
What we have to check is that $\omega_2(I_1 v,\cdot) = \omega_3(v,\cdot)$.
But
\begin{equation}
 \omega_2(I_1 v,\cdot) = g(I_2 I_1 v,\cdot) = g(-I_3 v, \cdot) = \omega_3(v,\cdot)
\end{equation}
as desired.
\end{pf}
...
give a \hk metric. You can obtain this as a consequence of the twistor
...
the $2$-form
\begin{equation}
\omega = \de Z_{\gamma_1} \wedge \de \bar{Z}_{\gamma_2} - \de Z_{\gamma_2} \wedge \de \bar{Z}_{\gamma_1}
\end{equation}
is \ti{positive}.
\end{itemize}
...
\end{equation}
for some constant $C'$. 
(This expectation can be made rigorous using the Poisson summation
formula, which moreover shows that the corrections are
exponentially suppressed in $\abs{z}$.)

Let us briefly consider the Gibbons-Hawking metric in this approximation, i.e. taking $V$ to be exactly given by
\eqref{eq:ooguri-vafa-V-approx}.
For $\abs{z} > e^{2 \pi(C' + C)}$
we will lose positivity, so the biggest base we can take is 
\begin{equation}
U = \{\abs{z} < e^{2 \pi(C' + C)}\} \times \R. 
\end{equation}
This Gibbons-Hawking metric is translation
invariant in the $x_3$ direction, and after dividing out by 
$\Z$ it becomes
one of the semiflat metrics of \autoref{exa:semiflat-1d-general}.
However, this metric has a bad singularity at $z=0$.

The \ti{exact} metric shares the bad large-$\abs{z}$ behavior
of the approximate one, but is better at small $\abs{z}$.
\fixme{...}
...
for some $c \in \R$.
where $c = \left(\int_X \omega_1^{2n}\right) / \left(\int_X \Omega \wedge \bar\Omega)^n \right). $
\end{prop}

\begin{pf} The main tool is the theorem of Yau, which says 
that there exists a unique Ricci-flat \kahler
metric $g$ on $X$ with $[\omega] = \alpha$.
Now let $I_1$ be the given complex structure on $X$,
and $\omega_1 = \omega$.
What we need to show is 
that $g$ is actually a \hk metric.
First, a Bochner argument shows that $\Omega$ is covariantly
constant. Thus, if we define 
\begin{equation}
  c = \frac{\omega_1^{2n}}{(\Omega \wedge \bar\Omega)^n},
\end{equation}
$c$ is a constant. Now define $\omega_2$ and $\omega_3$ by
$\omega_2 + \I \omega_3 = c \Omega$, and define
$I_2 = \omega_1^{-1} \omega_3$, $I_3 = \omega_2^{-1} \omega_1$.
