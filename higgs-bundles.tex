...
\begin{pf} \fixme{...} Here is the idea. 
...
\begin{pf} We need some results from deformation theory.
\fixme{give reference}
The deformations of a compact complex submanifold $Y \subset \cZ$
are ``controlled by'' the sheaf cohomology of the normal bundle.
...
$Y$ is $H^0(Y,N)$. We are going to apply this to the situation
where $Y$ is the image of one of the sections of $\cZ$.
Then $N \simeq \cO(1)^{\oplus 2n}$. The first bit of good 
...
$H^0(N)$. This is the \ti{complexified} tangent space to 
the desired $X$. Keeping track of real structures we see
that this pairing descends to a \ti{real} symmetric pairing
on the real tangent space. Then finally we have to check
