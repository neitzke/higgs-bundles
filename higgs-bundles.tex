...
We have
\begin{equation}
  \de \mu_1(\dot A, \dot \Phi) = - D \dot{A} - \dot\Phi \wedge \Phi - \Phi \wedge \dot\Phi
\end{equation}
...
\begin{align}
  ...
  &= - \int_C \Tr(Z D\dot A + Z(\dot \Phi \wedge \Phi + \Phi \wedge \dot \Phi) \\
  &= \de \mu_{1,Z}(\dot A, \dot \Phi)
\end{align}
as desired.
\end{pf}

Thus the vanishing of the \hk moment map gives \ti{Hitchin's equations}:
\begin{align*}
\bar\partial_D \Phi &= 0, \\
F_D + \Phi \wedge \Phi &= -2 \pi \I \frac{d}{K} \bid \omega_C.
\end{align*}

Let $\cA^{H,s}$ be the set of \ti{irreducible} solutions.

Now we can define
\begin{defn}[Moduli space of Higgs bundles]
\begin{equation}
