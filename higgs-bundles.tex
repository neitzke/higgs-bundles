...
$\cM_\C$ is the set of tuples $(D,\varphi,\bar\varphi)$,
...
Conversely, given a twistor line we need to show that it is of 
the above form. \fixme{is it really true?}
\end{pf}
...
Suppose $\vec\phi \in \cB'$ and $d' = 0$. Then $\rho_\C^{-1}(\vec\phi, \overline{\vec\phi}) = \Hom(H_1(\Sigma_{\vec\phi}),\C^\times)$.
\begin{pf} 
...
differential
$\tilde\phi_2 = \phi_2 - \frac14 \phi_1^2$ (rescaled discriminant).
