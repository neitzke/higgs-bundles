...
Suppose given a compact Riemann surface $C$ and compact reductive
Lie group $G$. Built from these data there is a
moduli space $$\cM = \cM^H(C,G)$$
which can be seen in various ways:
\begin{itemize}
\item $\cM$ is the \ti{character variety} i.e. moduli space of stable flat $G_\C$-local systems on $C$, i.e. moduli space of stable representations
...
over any $X$. This structure is much more rigid than that of 
a merely complex vector bundle.
