...
\subsection{Holomorphic symplectic forms}
...
form for a \hk structure on $X$. Note that this \hk structure does
...
\begin{example}[Ooguri-Vafa space]
\end{example}
...
\begin{prop}[Compact \kahler holomorphic symplectic manifolds are \hk] 
Suppose $X$ is a compact \kahler manifold with a holomorphic
symplectic form $\Omega \in \Omega^{2,0}(X)$. Fix any class
$\alpha \in H^2_{dR}(X)$ which is a \kahler class, i.e. there exists
\ti{some} \kahler metric on $X$ with $[\omega] = \alpha$.
Then there exists a unique \hk structure on $X$ such 
that $[\omega_1] = \alpha$ and $\omega_2 + \I \omega_3 = c \Omega$
for some $c \in \R$.
\end{prop}
