...
with the properties:
\begin{itemize}
\item There exists some $c \in \Gamma^*_\R$ such that
as $\zeta \to 0$ or $\zeta \to \infty$ we have
\begin{equation}
\fixme{...}
\end{equation}
\item $\cX$ is analytic away from the countable collection of rays
$$\ell_\mu = \{ \zeta: Z_\mu / \zeta \in \R_- \} \subset \C^\times $$
while on each of these rays $\xi$ jumps by the transformation
\begin{equation}
  \cX \to \cX \exp \left[ \sum_\mu DT(\mu) \mu^* \log(1 + \exp \xi(\mu)) \right].
\end{equation}
\end{itemize}
\end{lem}
