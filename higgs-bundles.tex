...
in the positive direction, $e^{-\I \vartheta} \sqrt{Q}$
is positive.
\end{defn}
...
$w$ given by $w(z) = \int^z_{z_0} \sqrt{Q}$. Then
we have $Q = \de w^2$. Thus in the coordinate $w$, 
$\vartheta$-trajectories are just straight segments 
of inclination angle $\vartheta$.
\end{pf}

\begin{prop}[Local singularities of the foliation by $\vartheta$-trajectories] Around each zero of $Q$, the foliation
by $\vartheta$-trajectories has a three-pronged singularity,
as shown in \fixme{fig}.
\end{prop}
...
is a loop on $\Sigma_{\vec \phi}$; the \ti{charge of $p$}
is the class of this loop in $H_1(\Sigma,\Z)$.
\end{defn}
