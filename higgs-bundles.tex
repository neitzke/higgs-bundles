...
\begin{exercise} Let $L$ be a line in $U$, oriented 
in the direction $\vec s = (1,0,0)$. 
Show that $\pi^{-1}(L)$ is a complex submanifold of $X$
in the complex structure $I_{\vec s}$. (Hint: it's sufficient
...
$I_1$. These correspond to $\theta = 0$ and $\theta = \pi$, 
...
Moreover show that this action is Hamiltonian with 
respect to all of the $\omega_i$, and compute the moment
maps $\mu_i: \R^4 \to \fu(1)^*$. 
If we identify $\fu(1) \simeq \R$, we obtain a map $\R^4 \to \R^3$ given by
...
in \autoref{exa:gibbons-hawking-R4}.
\end{exercise}
...
described in \autoref{exa:H-quotients}.
\end{example}
...
would have to be a point. But by looking at the fibers
of $z$ you can see they have no compact complex submanifolds.
It might be 
...
\begin{prop}[Holomorphic symplectic forms on a \hk manifold
vary holomorphically over $S^2$] Suppose $X$ is a \hk manifold. 
Let 
\begin{equation}
...
$\omega_2 - \I \omega_3 \in \Omega^{0,2}$.

For (2) here is a computational proof.
Fix a complex coordinate $\zeta$
on $S^2 \setminus \{ (-1,0,0) \}$, by
\begin{equation}
 \zeta = \frac{s_2 + \I s_3}{1 + s_1}, \qquad (s_1, s_2, s_3) = \frac{(1 - \abs{\zeta}^2, 2 \re \zeta, 2 \im \zeta)}{1+\abs{\zeta}^2}.
\end{equation}
Thus we have
\begin{align}
  \zeta = 0 \leftrightarrow \vec s = (1,0,0), \quad & \zeta = \infty \leftrightarrow \vec s = (-1,0,0), \\
  \zeta = 1 \leftrightarrow \vec s = (0,1,0), \quad & \zeta = -1 \leftrightarrow \vec s = (0,-1,0), \\
  \zeta = \I \leftrightarrow \vec s = (0,0,1), \quad & \zeta = -\I \leftrightarrow \vec s = (0,0,-1).
\end{align}
...
\begin{equation} \label{eq:hol-symp-family}
  \Omega(\zeta) = \frac{\omega_2 + \I \omega_3}{2\zeta} - \omega_1 + \frac{- \omega_2 + \I \omega_3}{2} \zeta.
\end{equation}
We want to know
%(see {\tt verify-holsymp.nb}) 
...
We can also give a more conceptual proof as follows. \fixme{explain better}
...
We have 
\begin{equation}
T^{0,1}_{(x,\zeta)} \cZ = \ker \Omega(\zeta,x) \oplus \IP{\partial_{\bar \zeta}}.
\end{equation}
To see the integrability, consider a pair of vector fields $v = v \oplus v'$
and $w = w \oplus w'$ with respect to this decomposition.
It is easy to see that $[v,w]$ lies in $T^{0,1} \cZ$ (this just comes from the
integrability on each fiber), and $[v',w']$ lies in $T^{0,1} \cZ$ 
(from the integrability on the $S^2$ base).

All that remains is to check
that $[\ker \Omega, \IP{\partial_{\bar \zeta}}] \subset T^{0,1} \cZ$.
So, suppose
$v$ is any section of $\ker \Omega \subset TX$ over $\cZ$.
Then also $[\partial_{\bar \zeta}, v]$ is a section of $TX$,
and we want to check that it lies in $\ker \Omega$.
Choose local coordinates $x^i$ for $X$, and write 
$v = v^i \partial_i$, $\Omega = \Omega_{ij}(\zeta) \de x^i \wedge \de x^j$:
then $[\partial_{\bar \zeta}, v] = (\partial_{\bar \zeta} v^i) \partial_i$,
and if $\Omega_{ij}(\zeta) v^i = 0$ then applying
$\partial_{\bar \zeta}$ gives $\Omega_{ij}(\zeta) \partial_{\bar \zeta} v^i = 0$,
which is what we want. \fixme{clean this up a little}
\end{pf}
...
\item $\cZ$ is a complex manifold, with a 
  ...
  The fiber $\pi^{-1}(\vec s)$ is just $(X,I_{\vec s})$ as a complex
  manifold. 
...
\begin{equation}
  \rho(x, \vec s) = (x, - \vec s).
\end{equation}
This trivial-looking bit of extra structure will turn out to be 
very important!
\item $\cZ$ carries a twisted fiberwise holomorphic symplectic form,
\begin{equation}
  \Omega \in \Omega^{2,0}_{fiber}(\cZ) \otimes \pi^* \cO(2) 
\end{equation}
(What this means: $\Omega^{2,0}_{fiber}(\cZ)$ is the \ti{quotient}
of $\Omega^{2,0}(\cZ)$ by the holomorphic subbundle
of forms $\de \zeta \wedge \alpha$,
$\alpha \in \Omega^{1,0}(\cZ)$.)
\end{enumerate}

\begin{example}[Twistor space of $\R^4$] \label{exa:R^4-twistor-space}
For the standard \hk structure on $\bbH$ all this becomes very 
concrete. The holomorphic $2$-form \eqref{eq:hol-symp-family}
can be written as
\begin{equation}
  \Omega(\zeta) = \frac{1}{2\zeta} \de z(\zeta) \wedge \de w(\zeta)
\end{equation}
where
\begin{align}
