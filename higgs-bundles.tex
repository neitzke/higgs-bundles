...
\begin{defn}[Moment map] \label{def:banach-moment-map}
The definition of moment map is the same as \autoref{def:moment-map}.
\end{defn}
...
$\Omega^1_k(\End E) = L^2_1(T^* \otimes \End E)$. 
...
\begin{prop}[Moment map for $\fG$ action on $\cA_k$] \label{prop:unitary-connections-moment-map}
The action of $\fG$ on $\cA_k$ admits a moment map, given by
...
\begin{prop}[Unitary gauge group acts properly]
...
unitary connections. Now we define:

\begin{defn}[Moduli spaces of unitary harmonic bundles]
\begin{equation}
  \cN^s_{K,d}(C) = \cA^s \kql{c} \fG, \qquad \cN_{K,d}(C) = \cA \kql{c} \fG.
\end{equation}
\end{defn}
...
\begin{thm}[Basic properties of $\cN_{K,d}(C)$] \label{thm:uharm-basics}
We have the following.
\begin{itemize}
\item $\cN^s_{K,d}(C)$ is a \kahler manifold, of complex dimension $(g-1)K^2+1$ where $g$ is the genus of $C$.
\item $\cN_{K,d}(C)$ is compact.
\end{itemize}
\end{thm}

\begin{pf}
First we state the \ti{formal} picture.
$\cN^s_{K,d}(C)$ arises as a symplectic quotient
by the ``effective gauge group''
\begin{equation}
 \fG^\eff = \fG / \C^\times
\end{equation} 
which acts freely (note that the moment 
map $\mu$ descends to this quotient!) Formally the tangent space
at a given flat connection $D$ should be the kernel of $\de \mu$ 
modulo $\rho(\Lie (\fG^\eff))$: in other words we want
the cohomology of the complex
\begin{equation}
  0 \to \Omega^0(\fu(E)) / \C \xrightarrow{D} \Omega^1(\fu(E)) \xrightarrow{D} \Omega^2(\fu(E)) \to 0
\end{equation}
at the middle position. (It's indeed a complex: $D^2 = 0$ acting
on sections of $\fu(E)$, since $F_D \sim \bid$ and hence 
is central.) 

We have to pay proper respect to the fact that we are dealing here
with \ti{infinite-dimensional} spaces. \fixme{describe the needed
Sobolev stuff}
Fortunately the complex
...
\kahler manifold.
\end{cor}
\begin{pf} This is just the combination of \autoref{thm:uharm-basics}
and \autoref{exc:coprime-semistable-is-stable}.
\end{pf}

\begin{example}[Jacobians]
The special case $K=1$ is very concrete. \fixme{...}
\end{example}
