...
It follows that $w(\zeta)$ and $z(\zeta)$ are fiberwise holomorphic.
...
Thus we can build coordinates by taking invariant 
...
\end{equation}
If we let $L$ denote the line bundle over $\cO(2) \to S^2$ with 
transition function $e^{\I \eta / \zeta}$, what we just showed
is that $\cZ$ is the total space of $L$ with the
zero section deleted. (Actually $L$ is holomorphically trivial,
but this description of it will turn out to be convenient.)
\end{example}
...
holomorphic coordinates as \fixme{typo here? recheck!}
\begin{align*}
...
\begin{equation} \label{eq:hol-symp-R2T2}
  \frac{\de \cX_A \wedge \de \cX_B}{\cX_A \cX_B} = \Omega(\zeta).
\end{equation}
