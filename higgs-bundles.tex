...
$\Z_k \subset U(1) \subset SU(2)_R$. This is a special case of the quotients
...
with two singularities. Take $U = \R^3$, fix distinct points
$x_1, x_2 \in \R^3$, and let 
\begin{equation}
  V(x) = \frac{1}{4\pi \norm{x - x_1}} + \frac{1}{4\pi \norm{x - x_2}}.
\end{equation}
The resulting $X$ is fibered over $\R^3$ with two degenerate fibers.
...
We can also give a more conceptual proof as follows.
Each complex structure $I_{\vec s}$ generates a 1-parameter
group of rotations, $U(1)_{\vec s} \subset SU(2)$. Indeed we can view
...
weight-$2$ piece. But this is precisely a standard construction 
of the holomorphic line bundle $\cO(-2)$ by ``induction'' from
the $U(1)$ subgroup (Borel-Weil theorem).
\end{pf}
...
\end{equation}
To see the integrability, suppose
...
\begin{equation}
  \Omega \in \Omega^{2,0}_{fiber}(\cZ) \otimes \cO(2) 
\end{equation}
