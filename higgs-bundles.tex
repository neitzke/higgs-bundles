...
\begin{prop}[Diagonal fibers of complexified Hitchin fibration] 
...
At least, get open neighborhood of compact torus in here}
\begin{pf}
...
Fix a countable collection of rays $\{\ell_\mu\}_{\mu \in \Lambda}$
...
\end{equation}
% \item For any phase $\vartheta$, $\cX(\zeta)$ admits analytic 
% continuation in the half-plane $\bbH_\vartheta$ 
% centered on $\arg \zeta = \vartheta$.
\item On the ray $\ell_\mu$, the limits from the two sides are 
related by \fixme{check signs}
\begin{equation}
...
\end{equation}
The jumps $S_\mu$ cancel out in $Y$, so $Y$ is analytic everywhere
\fixme{oops, why analytic and not just continuous?};
and our asymptotic condition says $Y$ is finite and $G_R$-valued 
in the limits
$\zeta \to 0$ or $\zeta \to \infty$. Thus by Liouville's theorem
$Y(\zeta)$ must be a $\zeta$-independent element of $G_R$. Finally
...
Everything we say in this section only involves the quadratic
differential
\begin{equation}
Q = \phi_2 - \frac14 \phi_1^2	
\end{equation}
(discriminant rescaled by $\frac14$).

\begin{defn}[$\vartheta$-trajectories of a quadratic differential]
A \ti{$\vartheta$-trajectory} of $Q$ is a connected 
real $1$-manifold $p$ on $C$
such that $e^{- \I \vartheta} \sqrt Q$ (with either choice of sign) is a
...
\begin{defn}[Orientation of $\vartheta$-trajectories]
Given a $\vartheta$-trajectory $p$, its lift $\tilde{p}$ to $\Sigma_{\vec\phi}$
is canonically oriented. Indeed, the two sheets of $\Sigma_{\vec\phi}$
correspond canonically to the two choices of $\sqrt{Q}$.
Thus we can orient $\tilde{p}$ by the condition that
in the positive direction, the form $e^{-\I \vartheta} \sqrt{Q}$
is positive.
\end{defn}
...
\begin{prop}[$\vartheta$-trajectories give a foliation] \fixme{...}
\end{prop}

\begin{prop}[Local singularities of the foliation by $\vartheta$-trajectories] \fixme{...}
\end{prop}


\begin{defn}[Saddle connections]
A \ti{$\vartheta$-saddle connection} is a $\vartheta$-trajectory $p$
such that $\bar p \setminus p$ consists of two points.
\end{defn}
...
\begin{lem}[Phase constraint for saddle connections]
If a $\vartheta$-saddle connection exists with charge
...
In particular, $\vartheta$-saddle connections can only 
occur at countably many phases.
