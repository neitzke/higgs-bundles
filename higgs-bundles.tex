...
\end{equation}
\end{thm}
\begin{pf} Follow the pattern of the proof
of \autoref{prop:symplectic-quotient-is-symplectic}.
The infinite-dimensional setting leads to the following
hiccups:
\begin{itemize}
\item 
To show that $\mu^{-1}(0)$ is a submanifold of $X$,
we want as before to show that $0 \in W$ is a regular value for $\mu$.
This will not be true for $\mu: X \to \fg^*$ but will be true
for $\mu: X \to W$. Indeed we know that for any $Z \in \fg$
we have $\de \mu_Z = \iota_{\rho(Z)} \omega \neq 0$ as before.
Thus $\im \de \mu$ is not annihilated by any $Z \in \fg$.

Since $W \subset \fg^*$ is dense this shows that 

\end{itemize}
\end{pf}


The point of the peculiar hypothesis on $W$ in 
\autoref{prop:banach-symplectic-quotient-is-symplectic} is that,
in our applications below, $\mu$ is simply \ti{not} surjective onto
$\fg^*$ in the usual sense: it is surjective only onto some dense
subspace. Fortunately $\mu$ in our application has good
regularity properties which allow us to define a stronger norm;
in that stronger norm it actually has closed image.
...
\begin{equation} \label{eq:gauge-complex}
  0 \to \Omega^0_{k+1}(\fu(E)) \xrightarrow{D} \Omega^1_k(\fu(E)) \xrightarrow{D} \Omega^2_{k-1}(\fu(E)) \to 0
\end{equation}
...
Fortunately this complex
is \ti{elliptic} in the sense of \autoref{sec:ellipticity}.
Thus we get:

\begin{prop}[Elliptic regularity for the gauge complex]
Define
\begin{equation}
 \hat{D} = D \oplus D^*: \Omega^1_k \to \Omega^2_{k-1} \oplus \Omega^0_{k-1}.
\end{equation}
...
\begin{lem}[Cohomology vanishing for the gauge complex] 
\label{lem:cohomology-vanishing-gauge-complex}
If $D \in \cA^s_k$, then
$H^0$ and $H^2$ of the complex \eqref{eq:gauge-complex} are both
$1$-dimensional.
\end{lem}
\begin{pf}
Any $D$-constant section of $\fu(E)$ must be a constant multiple
of $\bid$, since $D$ is an irreducible connection: this gives
the desired statement for $H^0$. For $H^2$ we essentially
use Poincare duality. More precisely, we need to use the
Fredholm property of $\hat D$, which tells us that $H^2$ can 
be identified with $\ker D^* \subset \Omega^2_{k-1}(\fu(E))$.
On the other hand we have
\begin{equation}
  D^* = - \star D \star
\end{equation}
so any $\alpha \in H^2$ has $D \star \alpha = 0$; said
