...
\end{prop}
\begin{pf}
From the fact that $G$ acts freely on $\mu^{-1}(0)$,
$\omega$ is nondegenerate, and \eqref{eq:moment-map-condition-3},
it follows that $0$ is a regular value of $\mu$;
thus $Y = \mu^{-1}(0)$ is a submanifold of $X$,
with $\dim Y = \dim X - \dim G$.
Then $X \kq G = Y / G$ is also a manifold of the desired 
dimension.

It remains to check that $\omega$ descends to a
symplectic form. We want to \fixme{...}
\end{pf}
...
Suppose $X$ is a \kahler manifold, with a group
...
Take $X = \C^n$ with its standard symplectic form
\begin{equation}
...
Yau's theorem (solution of the Calabi conjecture) 
