...
$\lambda = p_i \de q^i$, or more invariantly, for $v \in TY$
we have $\lambda(x,p) \cdot v = p \cdot \pi_* v$.
Then there is a canonical symplectic form $\omega = \de \lambda$:
in coordinates it would be $\omega = \de p_i \wedge \de q^i$.
\end{example}
...
Conversely, if a moment map $\mu$ does exist and $Z(\fg)$
is nontrivial, we can get
another moment map by taking 
$\mu + c$ where $c \in Z(\fg)$.
...
Letting $H \subset TX$ be the space of horizontal vectors,
using orthogonal projection we obtain
...
Suppose $X$ is a Riemannian manifold, with a compact group
$G$ acting freely on $X$ preserving $g$. 
The \ti{induced metric} on $X/G$ is just $g \vert_H$
(where we identified $T(X/G)$ with the space of horizontal
vectors $H$ as above.)
\end{defn}
...
gives a metric-compatible and torsion-free connection. \fixme{...}) Then 
...
There is another way of thinking about this example: instead of
taking $\C^n \kq U(1)$ we could try to take $\C^n / \C^\times$.
Actually this quotient is badly behaved as it stands, because of the
point $0 \in \C^n$, but we certainly do have
\begin{equation}
  (\C^n \setminus \{0\}) / \C^\times = \C\PP^{n-1} = \C^n \kq U(1).
\end{equation}
The explanation is that each 
$\C^\times$-orbit contains exactly one $U(1)$-orbit
lying inside $\mu^{-1}(0)$, except for the orbit $\{0\}$.

This is part of a general story: at least 
in good situations, the \kahler quotient by a group $G$ 
is related to the ordinary quotient by a \ti{complexification}
$G_\C$. Such a quotient is typically badly behaved since $G_\C$ is
not compact; but it becomes
better behaved after throwing away some ``bad'' points, like
the point $0 \in \C^n$ in the above example.
...
\begin{equation} \label{eq:V-ooguri-vafa-resummed}
  V = -\frac{1}{2\pi} \log \abs{z/\Lambda} + \frac{1}{\pi} \sum_{n=1}^\infty \cos (2 \pi n x_3) K_0(2 \pi n \abs{z})
\end{equation}
...
where $V$ is exactly given by
$V^\sf$ of \eqref{eq:ooguri-vafa-V-approx}.
For $\abs{z} > \Lambda$
...
However, this metric seems very unlikely to admit a reasonable
extension over $z = 0$.

The Gibbons-Hawking metric obtained from the full $V$ 
...
parameter controlling this gluing.}
...
\autoref{exa:semiflat-1d-general}, where $\Gamma$ is a rank $2$
local system of lattices, whose monodromy around the origin
...
Ooguri-Vafa space we could use the function written 
...
which combine to give $\cX_2 \to \cX_2 \cX_1$.
(Compare the result of \autoref{exc:ooguri-vafa-semiflat}.)

The function $\cX_2^\sf$ is \ti{not} holomorphic on the
exact Ooguri-Vafa space. However it turns out that we can
find a holomorphic function which has the same \ti{asymptotic}
behavior as $\zeta \to 0,\infty$.
Namely we write
\begin{equation}
...
shift $x_3 \to x_3 + 1$. All of the constructions then go through as 
before and give
...
\begin{thm}[\Hk quotients are \hk] Suppose $X$ is \hk and a
...
\begin{equation}
\de M_1 \cdot Z = \iota_{\rho(Z)} \Omega_1 \in \Omega^{1,0}(X),
\end{equation} 
so $M_1$ is holomorphic.
Thus $Y = M_1^{-1}(0)$ is an $I_1$-complex
submanifold. Now $G$ acts on $Y$
...
So this single \hk quotient space can be viewed as a \kahler 
quotient in many different ways. There is also yet another way to think of
it, using the fact that the \kahler quotient is also ``like''
a quotient by a complexification $G_\C$. From this point of view
we could write formally
\begin{equation}
  X \hkq G = M_1^{-1}(0) / G_\C = X \kq G_\C
\end{equation}
i.e. it looks like a \ti{holomorphic} version of the usual
symplectic quotient. This point of view is useful for 
...
We begin with the space $X = \bbH^{n+1}$. 
In structure $I_1$, $X$ is naturally identified with
$\C^{2n+2}$ with coordinates $(z_1, w_1, \dots, z_{n+1}, w_{n+1})$.
It has a triholomorphic $U(1)^{n+1}$ action (coming from $SU(2)_R$ action on 
each factor), acting by $z_i \to e^{\I \alpha_i} z_i$,
$w_i \to e^{-\I \alpha_i} w_i$, which has a standard moment map 
$X \to \R^3 \otimes_\R \fu(1)^{n+1} \simeq \R^{3n+3}$, given by \fixme{check sign}
\begin{align}
  \mu_{i,1} &= \half (\abs{w_i}^2 - \abs{z_i}^2), \\
  \mu_{i,2} + \I \mu_{i,3} &= w_i z_i.
\end{align}
Now consider the ``center of mass'' subgroup $G \subset U(1)^{n+1}$
given by the condition $\prod_i g_i = 1$.
We have $G \simeq U(1)^n$.
Choose points $\vec c_1, \dots, \vec c_{n+1} \in \R^3$ and 
consider the \hk quotient
\begin{equation}
  Y = X \hkq_{\vec c} G.
\end{equation}
Claim: this quotient is actually a multi-Eguchi-Hanson space
as in \autoref{exa:multi-eguchi-hanson}, where the 
differences of the $\vec c_i$ are the positions $x_i$.
\fixme{...}
\end{example}
...
We are going to define a moduli space $\Bun_{K,d}(C)$ parameterizing
...
$E = \bigoplus_{i=1}^k \cO(n_i)$.
The slope of such a bundle is
\begin{equation}
\mu(E) = \frac{\sum n_i}{k}.  
\end{equation}
Thus $E$ is unstable unless
all $n_i$ are equal; if all $n_i$ are equal it is
semistable; it is stable only if $k<2$.
...
to \fixme{check}
\begin{equation}
  \begin{pmatrix} z & t \\ 0 & z^{-1} \end{pmatrix}
\end{equation}
...
All rank $K$ complex vector bundles of degree $d$ are equivalent.
...
$\bar\partial_E$.
Thus the moduli space we want to describe could be thought of as
the (infinite-dimensional) space $\cA$ of 
all $\bar\partial_E$, modulo
equivalence --- where now ``equivalence'' means
...
Now we look again at the space $\cA$. Each $\bar\partial_E \in \cA$
can be extended in a unique way to a unitary connection
on $E$ (Chern connection), and conversely, each 
unitary connection can be restricted to a $\bar\partial_E$ operator.
Thus, viewed this way, $\cA$ becomes
simply the space of all unitary
connections on $(E,h)$.

$\cA$ is an infinite-dimensional affine space, modeled
on the linear space $\Omega^{1}(\fu(E))$.
On the latter space we have a natural symplectic form
\begin{equation}
  \omega(\alpha,\beta) = \frac{1}{8 \pi^2} \int_C \Tr(\alpha \wedge \beta).
\end{equation}
So this gives a symplectic structure on $\cA$.
Moreover $\fG$ acts on $\cA$.

\begin{prop}[Moment map for $\fG$ action on $\cA$]
The action of $\fG$ on $\cA$ preserves the symplectic 
structure and admits a moment map, given by
\begin{equation}
  \mu(D) = F_D.
\end{equation}
(More precisely, this means: suppose given an element $Z \in \Lie \fG$,
represented by a smooth section of $\End E$; 
then we have $\mu_Z(D) = \int_C Z F_D$.)
\end{prop}

Thus at least formally we should expect that $\mu(D)$
