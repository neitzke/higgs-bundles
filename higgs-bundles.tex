...
Note that
\begin{equation}
  \bar\partial_D = (-2 (\im \tau)^{-1} \alpha) \de \bar{z} 
\end{equation}
...
\end{equation}
(Comparing with our previous notation for the general case, 
...
\begin{equation}
  \nabla_\zeta = D - \zeta^{-1} \varphi - \zeta \varphi^\dagger
\end{equation}
...
So in fact $\cM_{K,d}(C)$ can be identified with 
...
\begin{example}[Moduli of Higgs line bundles over a torus]
\fixme{...}
\end{example}

Now what are the holomorphic functions on this \hk space?
\fixme{holonomy functions}
