...
$$ \DIP{\cdot,\cdot}: \Omega^1(\Gamma^*_\C) \times \Omega^1(\Gamma^*_\C) \to \Omega^2(\Gamma^*_\C). $$
...
on $\Sigma_{\vec \phi}$, which says
$$ \DIP{\alpha,\beta} = \int_{\Sigma_{\vec\phi}} \alpha \wedge \beta
$$
where on the left we view $\alpha, \beta$ as elements 
in $\Gamma^*$ by integration. Thus we get
$$ \DIP{\de Z(v), \de Z(v')} = \int_{\Sigma_{\vec\phi}} \iota_{s_v} \Omega \wedge \iota_{s_{v'}} \Omega = 0 $$
for degree reasons.
\end{proof}
...
degree $d' = d - K(K-1)(g-1)$. In particular it is not immediately obvious
how to identify nearby torus fibers. To deal with this problem we
\fixme{...}
% specialize to the case $d=0$. In this case, 
% the torus fiber $\rho^{-1}(\vec\phi)$ is canonically identified
% with the moduli space of flat $U(1)$-connections over 


% Now we make a slightly ugly choice: we fix a contractible patch $U \subset \cM'$
% and an arbitrary section of the Hitchin fibration over $U$.
% Having made this choice, we can identify 
