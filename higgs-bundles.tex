...
bundle $E$ carrying a flat connection $D$.
...
just one step, and $\gr E = E$. More generally we have the 
following:
...
In particular,
\begin{align}
  \Phi \wedge \Phi = [\Phi_x, \Phi_y] \de x \wedge \de y,
\end{align}
...
\begin{exercise} Show that
\begin{equation}
  \mu_2 = D \Phi, \qquad \mu_3 = D \star \Phi.
\end{equation}
\end{exercise}
...
and the second is
\begin{equation}
(\dot A, \dot \Phi) \mapsto (D \dot{A} + [\Phi,\dot\Phi], D \dot\Phi + [\dot A, \Phi], D \star \dot\Phi + [\dot A, \star \Phi]).
\end{equation}
...
we should interpret the quotient by the image of $\Omega^0$
by taking the orthocomplement, i.e. imposing the ``gauge-fixing'' 
condition \fixme{check sign}
$D^* \dot{A} + \star [\dot\Phi, \Phi] = 0$.
So altogether the tangent space we are after is the kernel of
the operator
\begin{align}
\hat{D}: \Omega^1(\fu(E))^{\oplus 2} &\to \Omega^2(\fu(E))^{\oplus 3} \oplus \Omega^0(\fu(E)) \\
(\dot A, \dot \Phi) &\mapsto \begin{pmatrix} D \dot{A} + [\Phi,\dot\Phi] \\ D \dot\Phi + [\Phi, \dot A] \\ D \star \dot\Phi + [\star \Phi, \dot A] \\ D^* \dot{A} + \star[\Phi, \dot \Phi] \end{pmatrix}
\end{align}
...
of $\Omega^2(\fu(E))^{\oplus 3} \oplus \Omega^0(\fu(E))$.
\end{lem}
...
Unlike $\cN_{K,d}(C)$, it is hopeless to ask for
$\cM_{K,d}(C)$ to be compact. We will see this in the next example.
