...
vary holomorphically over $S^2$] Suppose $X$ is a \hk manifold. For each $\vec s \in S^2$, let
\begin{equation}
 L_{\vec s} \subset \Omega^2_\C(X)
\end{equation}
be the vector space spanned by $2$-forms
$\omega_{\vec s'} + \I \omega_{\vec s \times \vec s'}$, where
$\vec s' \perp \vec s$. Then:
\begin{enumerate}
\item $L_{\vec s}$ is a $1$-dimensional complex subspace. 
\item Any $\Omega \in L_{\vec s}$ is a holomorphic symplectic form
on $(X,I_{\vec s})$.
\item If we equip $S^2$ with its standard 
complex structure, $L$ is a \ti{holomorphic} line subbundle of the trivial holomorphic bundle $S^2 \times \Omega^2_\C(X)$, isomorphic to $\cO(-2) \to \C\PP^1$.
\end{enumerate}
\end{prop}

\begin{pf} For (1) note that any two $\vec s'$ orthogonal to $\vec s$
differ by a rotation $R$ around $\vec s$ by some angle $\alpha$,
and then compute directly that such a rotation just changes
$\omega_{\vec s'} + \I \omega_{\vec s \times \vec s'}$ by
a factor $e^{\I \alpha}$.

(2) is a direct computation, essentially the same as the proof
of \autoref{prop:hyperkahler-is-hol-symplectic}.

For (3) here is a computational proof.
...
\end{align}
Then consider the family of 2-forms
\begin{equation}
  \Omega(\zeta) = \frac{\omega_2 + \I \omega_3}{2\zeta} - \omega_1 + \frac{- \omega_2 + \I \omega_3}{2} \zeta.
\end{equation}
Now we verify that $\Omega(\zeta) \in L_\zeta$.

For any $\zeta \in \C$, $\zeta \Omega(\zeta) \in L_\zeta$;
similarly, for any $\zeta \in \C^\times \cup \{\infty\}$,
$\Omega(\zeta) / \zeta \in L_\zeta$.
Together these give the desired holomorphic identification
between $L_\zeta$ and $\cO(-2)$.
\end{pf}
