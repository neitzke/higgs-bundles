...
Suppose given a compact Riemann surface $C$ and compact semisimple
Lie group $G$. Built from these data there is a
moduli space $$\cM = \cM^H(C,G)$$
which is seen in different ways by different people:
\begin{itemize}
\item $\cM$ is a partial compactification of $T^* \Bun(C,G)$, where $\Bun(C,G)$ is the moduli space of stable $G$-bundles on $C$; from this point of view Lagrangian submanifolds are related to \ti{D-modules} on $\Bun(C,G)$, basic objects on one side of the geometric Langlands correspondence,
\insfig{higgs-bundles-2}{0.55}
\item $\cM$ is a \ti{complex integrable system}, i.e. a holomorphic
...
\item $\cM$ is the \ti{character variety} i.e. moduli space of flat $G_\C$-connections on $C$; certain sheaves on this moduli space are 
basic objects on the other side of the geometric Langlands correspondence,
...
philosophy, which says that every Calabi-Yau manifold arises naturally
...
$^L G$ is the \ti{Langlands dual} group, and the mirror symmetry
exchanges the two sides of the geometric Langlands correspondence,
\item $\cM$ is the space of solutions of an interesting PDE, \ti{Hitchin's equations}, containing as special cases various sorts of harmonic maps (including \ti{uniformization} in the case $G = PSU(2)$).
...
How can one space be so many different things at once?
...
endomorphism of the underlying real vector space.
...
obeying
\begin{equation}
  \bar\partial_E (\alpha \psi) = (\bar\partial_E \alpha) \psi + (-1)^{\abs{\alpha}} \alpha \wedge \bar\partial_E \psi \qquad \alpha \in \Omega^*(X), \quad \psi \in \Omega^0(E)
\end{equation}
...
all of $I$, $J$, and $K$.
We call this subgroup $SU(2)_L$, and the other one
$SU(2)_R$.
\end{exercise}
...
$\Theta$ (represented as a $1$-form on $X$) 
whose curvature is $F$. Then the metric
on $X$ given by \fixme{check factors}
\begin{equation}
