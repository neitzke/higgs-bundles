...
as a \ti{fiberwise} holomorphic symplectic quotient.

\begin{example}[Multi-Gibbons-Hawking spaces as \hk quotients]
We begin with the space $\bbH^{n+1}$. 
In structure $I_1$ it is naturally identified with
$\C^{2n+2}$ with coordinates $(z_1, w_1, \dots, z_{n+1}, w_{n+1})$.
It has a triholomorphic $U(1)^n$ action (coming from $SU(2)_R$ action on 
each factor)
\fixme{...}
Now choose points $x_1, \dots, x_{n+1} \in \R^3$. We use these to 
define a moment map for the $U(1)^{n+1}$ action \fixme{...}
\end{example}
