...
fiber coordinate $x_{0,\alpha} \in \R / 2 \pi \Z$, then $\Theta$ 
is locally of the form
\begin{equation}
 \Theta = A_\alpha + \de x_{0,\alpha} 
\end{equation}
...
By the maximum principle, there are no 
nonconstant positive harmonic functions on the full $\R^3$.
Thus \autoref{exa:incomplete-gibbons-hawking} does not give
any interesting complete \hk metrics. However we can do better:
...
(Hint: the subset of $X$ with $\norm{x-x_0} = r$ is
the Hopf fibration over $S^2$, thus homeomorphic to
$S^3$; the restriction of $g$ to this $S^3$ 
turns out to be the round metric with 
radius $\rho = \sqrt{r}$, while $g$ in the radial 
direction is $\de r^2 / r = \de \rho^2$.)
\end{exercise}
