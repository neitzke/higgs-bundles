...
As we did with the moduli spaces of Einstein connections, we 
...
Now, we have already studied the whole family of complex
structures $I_\zeta$ in this example: for any
$\zeta \in \C^\times$,
...
\subsection{The role of stability}
...
in this doubled context.

The correct statements are:

\begin{thm}[Polystable Higgs bundles admit harmonic connections] \label{thm:polystable-higgs-bundles-harmonic}
Suppose $(E, \bar\partial_E, \varphi)$ is a Higgs bundle, with $\fG_\C$-orbit
$\cO$. Then $\cO \cap \mu_1^{-1}(0)$ 
consists of a single $\fG$-orbit if $(E, \bar\partial_E, \varphi)$ is polystable, and is empty otherwise. $(E,\bar\partial_E,\varphi)$ is
stable iff the corresponding harmonic pair is irreducible.
\end{thm}

\begin{thm}[Reductive connections can be made harmonic]
Suppose $(E, \nabla)$ is a flat complex connection on $E$, with $\fG_\C$-orbit
$\cO$.
Then $\cO \cap \mu_\zeta^{-1}(0)$ consists of a single $\fG$-orbit if
$(E, \nabla)$ is reductive, and is empty otherwise. $(E,\nabla)$ is irreducible
iff the correspnding harmonic pair is irreducible.
\end{thm}

Thus, the formal picture we set up becomes precise: \fixme{...}

Now we consider a fun example, which needs a little bit of setup.
...
\begin{example}[Uniformization theorem via Higgs bundle] Fix a spin structure $\cL$ 
on $C$. We consider the stable Higgs bundle
\begin{equation}
...
Higgs bundle. In other words, there exists \fixme{...}
...
vector space
\begin{equation}
  \cB = \bigoplus_{i=1}^K H^0(C, K_C^{\otimes i}).
\end{equation}
\end{defn}
...
\begin{defn}[Spectral curve] \fixme{...}
\end{defn}

\begin{prop}[$\cM$ is fiberwise compactification of $\cN$]
...
of $T^* \cN_{K,d}^s(C)$. But it is hopeless to ask for
$\cM^s_{K,d}(C)$ or $\cM_{K,d}(C)$ to be literally compact.
A partial substitute is:

\begin{prop}[Completeness of \hk metric on $\cM_{K,d}^s(C)$] The \hk metric on $\cM_{K,d}^s(C)$ is complete.
\end{prop}
