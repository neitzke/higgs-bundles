...
What structure would the moduli space then get?
...
We recall that, ignoring questions of stability, we have
\begin{equation}
	\cN_{K,d}(C) = \cA^\C_{K,d} / \fG_\C.
\end{equation}
This suggests that we might profitably view $T^* \cN_{K,d}(C)$
as a \ti{holomorphic} symplectic quotient,
\begin{equation}
	T^* \cN_{K,d}(C) = (T^* \cA^\C_{K,d}) \kq \fG_\C.
\end{equation}
...
often has another interpretation: if the space $X$ with which we began
was actually \hk, then we can try to identify $X \kq G_\C = X \hkq G$,
...
\begin{exercise} Verify that indeed $\Omega_1 = \omega_2 + \I \omega_3$.
\end{exercise}
...
$V \oplus V^*$ admits a canonical \hk structure, for which $I_1 = I \oplus I^*$,
$\omega_1 = \omega \oplus \omega^{-1}$,
and $\Omega_1(v \oplus \alpha, v' \oplus \alpha') = \alpha'(v) - \alpha(v')$. \fixme{check factors}
\end{exercise}
