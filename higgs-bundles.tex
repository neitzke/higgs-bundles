...
\begin{example}[Some non-reductive connections]
...
It is interesting in this case to consider the corresponding family of
flat connections: \fixme{...}
...
whose sheets over $z \in C$ are the $K$ eigenvalues of $\varphi(z)$.
...
of the equation \eqref{eq:spectral-curve}. For example, if $K=2$
then the equation is $\lambda^2 + \phi_1 \lambda + \phi_2 = 0$,
which has discriminant
\begin{equation}
  \Delta_{\vec\phi} = \phi_1^2 - 4 \phi_2.
\end{equation}
Note that globally over $C$ this is a section of $K_C^2$.
More generally, the formula
\begin{equation}
  \Delta = \prod_{i > j} (\lambda_i - \lambda_j)^2
\end{equation}
shows that $\Delta_{\vec\phi}$ is a section of $K_C^{K(K-1)}$.
The zeroes of $\Delta_{\vec\phi}$ are the places on $C$ where the 
sheets $\lambda_i$ collide, i.e. they are the places where
curve $\Sigma_{\vec\phi}$ is \ti{ramified} as a cover of $C$.

The simplest behavior occurs at a \ti{simple} zero of $\Delta_{\vec\phi}$.
Indeed at such a zero the local behavior is like that of the 
...
\begin{prop}[Generic fibers of the Hitchin fibration are shifted Jacobians]
...
\begin{prop}[Hitchin fibration is proper and surjective] The Hitchin fibration
...
But given the surjectivity of $\rho$
