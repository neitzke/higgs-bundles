...
\chead{\tiny \ti{Moduli of Higgs Bundles, \ti{preliminary} and \ti{incomplete} draft}}
...
at
\begin{center}
\small \tt{www.ma.utexas.edu/users/neitzke/teaching/392C-higgs-bundles/higgs-bundles.pdf}
\end{center}
Please send corrections/improvements to
\begin{center}
\small \tt{neitzke@math.utexas.edu}
\end{center}
or as pull requests to the source repository hosted at
\begin{center}
\small \tt{github.com/neitzke/higgs-bundles}
\end{center}
}
...
smooth manifold. (Later we will occasionally need to talk about 
...
Suppose $X$ is a smooth manifold with $\dim_\R X = 2n$, 
...
Unlike the finite-dimensional situation, even linear maps
...
We say an antisymmetric bilinear pairing 
$\omega: V \times V \to \R$ is a \ti{nondegenerate $2$-form}
if it is continuous in each factor separately and the map
\begin{align}
  V &\to V^* \\
  v &\mapsto \iota_v \omega = \omega(v,\cdot)
\end{align}
...
intuition is:
\begin{prop}\label{Degree counts zeroes} Suppose $E$ is a complex vector bundle over $C$, of rank $K$.
...
which $\fG$ acts freely and properly on $\mu^{-1}(0)$.
...
The action of $\fG$ on $\cA$ is proper.
\end{prop}
\begin{pf} \fixme{...}
\end{pf}
...
\begin{thm}[Basic properties of $\uharm_{K,d}(C)$] \label{thm:uharm-basics}
We have the following.
\begin{itemize}
\item $\uharm^s_{K,d}(C)$ is a \kahler manifold, of complex dimension $(g-1)K^2+1$ where $g$ is the genus of $C$.
...
\begin{cor}[$\uharm_{K,d}(C)$ is nice when $(d,K)$ = 1]
When $(d,K) = 1$, $\uharm_{K,d}(C)$ is a compact
\kahler manifold.
\end{cor}
...
\begin{prop}[Formula for the \kahler metric on $\uharm_{K,d}(C)$]
\fixme{...}
\end{prop}
...
\begin{equation}
\uharm^s_{K,d}(C) \simarrow \uharm^s_{K,d+Kd'}(C).
\end{equation}
\end{prop}
\begin{pf}
