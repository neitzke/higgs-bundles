...
\end{equation}
A general flat connection
is gauge equivalent to one of the form $D = \de + A$ with
\begin{align}
  A &= \theta_x \, \de x + \theta_y \, \de y \\
  &= (2 \im \tau)^{-1} (-\I \bar\alpha \, \de z + \I \alpha \, \de \bar{z})
\end{align}
where
\begin{equation}
...
the coefficient $2 \I (\im \tau)^{-1} \alpha$,
so $\alpha$ is a holomorphic coordinate.  
...
This gives the desired $f$. \fixme{unique up to scalar multiple}
\end{pf}
...
\begin{pf} We just discuss the case $d=0$, only the stable case,
and moreover we only give a vague sketch,
...
$\abs{F_{D_n}} \to 0$. Using the Uhlenbeck compactness
...
$D^* \in \cO$ then we are done. If not, then we show
that the original holomorphic bundle is actually unstable.
