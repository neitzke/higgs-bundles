...
\begin{defn}[Complemented subspace] \fixme{...}
\end{defn}
...
Suppose $X$ is a Banach manifold with a Banach Lie group $G$
acting properly and freely.
Suppose also that the tangent spaces to orbits, 
$T_x (Gx) \subset T_x X$, are closed and complemented.
Let $\fg = \Lie G$ and let
\begin{equation}
\rho: \fg \to \Vect(X)
\end{equation}
...
\begin{itemize}
\item The quotient $X / G$ (the set of $G$-orbits on $X$) 
has a natural structure of Banach manifold, compatible with
the quotient topology.
\item The differential of the projection $X \to X/G$ 
is surjective and gives an isomorphism
\begin{equation}
...
If $G$ acts freely on $\mu^{-1}(0)$, \fixme{are we sure this is really
sufficient condition? if so then we are saying that things are
exactly as in the finite-dimensional situation!}
...
Altogether we now have a single space $\cA_k$ with both symplectic and
complex structures.
...
\end{exercise}
Thus $\cA_k$ is \kahler.
Moreover $\fG_{k+1}$ acts on $\cA_k$ by conjugation,
\begin{equation}
...
is just wedging with $p$.} This has the following encouraging
consequences:
