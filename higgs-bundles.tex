...
in $\R^4 \setminus \{0\}$. This means that $X$ can be extended
...
There is a more conceptual way of arriving at this construction.
For this we need a definition, which we will also 
use in the future:
\begin{defn}
Suppose $X$ is a symplectic manifold, with symplectic form $\omega$,
acted on by a Lie group $G$, with Lie algebra
$\fg$. By differentiating this action we obtain a map
\begin{equation}
  \rho: \fg \to \Vect(X).
\end{equation}
We say a function 
\begin{equation}
 \mu: X \to \fg^* 
\end{equation}
is a \ti{moment map} for this action if 
for all $Y \in \fg$ we have
\begin{equation}
  \rho(Y) = \omega^{-1}(\de (\mu \cdot Y)),
\end{equation}
i.e. for any $v \in \Vect(X)$
\begin{equation}
  \omega(\rho(Y), v) = v (\mu \cdot Y).
\end{equation}
If the action of $G$ admits a moment map, then we call
this action \ti{Hamiltonian}.
\end{defn}
...
Show that the rotation action of $U(1)$ on $X$ is Hamiltonian, 
with moment map $\mu: \R^2 \to \fu(1)^*$ given by
\begin{equation}
