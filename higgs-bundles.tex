...
There is also a less computational approach to \autoref{lem:hol-symp-hol}
and \autoref{lem:10-forms-hol}, as follows.
...
\item $\eps_E$ is covariantly constant for $\nabla_E$.
\item The \hk metric $g$ is \fixme{check factors}
\begin{equation}
  g(h_1 \otimes e_1, h_2 \otimes e_2) = \omega_H(h_1, h_2) \omega_E(e_1, e_2).
\end{equation}
...
structure $I_h$ on $X$.
Moreover the map $h \mapsto \Omega_h$
...
The projection $\Sym^2(H) \setminus \{0\} \to \PP(H)$ realizes
$\Sym^2(H) \setminus \{0\}$ as the complement of the zero
section in $\cO(-2) \to \C\PP^1$. Thus $\Omega_h$ defines an 
...
on $X$ is all the data of a \hk structure except that we 
...
canonical pseudo-\hk structure. 
If $\cZ$ is the twistor space of a
\hk manifold $X'$, then $X' \subset X$, and the \hk structures
agree.
\end{thm}

\begin{pf} \fixme{need a little deformation theory to set this up!}
\end{pf}
...
\end{equation}
Now fix a local basis $\{\gamma_1, \dots, \gamma_n\}$ of $\Gamma$
over $U \subset \cB$, let $\eps_{ij} = \IP{\gamma_i, \gamma_j}$, 
let $\eps^{ij}$ be the inverse of the matrix $\eps_{ij}$
($\eps^{ij} \eps_{jk} = \delta^i_k$),
and finally define $\Omega(\zeta) \in \Omega^2_\C(X\vert_U)$
by
\begin{equation}
  \Omega(\zeta) = \I \sum_{i,j = 1}^n \eps^{ij} \frac{\de \cX_{\gamma_i}(\zeta) \wedge \de \cX_{\gamma_j}(\zeta)}{\cX_i(\zeta) \cX_j(\zeta)}.
\end{equation}
Although we defined it over a patch $U \subset \cB$,
the $\Omega$ obtained is independent of the choice of
basis $\{\gamma_i\}$, and thus it extends to a form
...
\kahler manifold in structure $I_1$. The canonical bundle $K$
...
\begin{prop}[Compact, \kahler, uniquely holomorphic symplectic, simply connected manifolds are uniquely \hk]
Suppose that:
\begin{itemize}
