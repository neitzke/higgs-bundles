...
over $\R$ (resp. $\C$),
$V^*$ is the space of all bounded linear maps $V \to \R$
...
We say $f$ is \ti{differentiable} at $v$, and $\de f_v: V \to W$ is its
\ti{derivative} at $v$, if
\begin{equation}
  \lim_{h \to 0} \frac{\norm{f(v+h) - f(v) - \de f_v(h)}}{\norm{h}} = 0.
\end{equation}
\end{defn}

\fixme{better also define smooth}

\begin{thm}[Inverse function theorem for Banach spaces]
Suppose $f: V \to W$ is a smooth map of Banach spaces, and for some
$v \in V$ the map $\de f_v: V \to W$ is a bounded isomorphism.
