...
\end{thm}
\begin{pf} First note that if $G$ acts freely on 
$\vec\mu^{-1}(0)$ then $0$ is a regular value
for $\vec\mu$, so that $Y = \vec\mu^{-1}(0)$ is
a manifold of dimension $\dim_\R X - 3 \dim_\R G$, 
with $G$ acting freely.
Thus $X \hkq G = Y / G$ is a manifold, of the desired dimension.
...
submanifold (hence in fact \kahler). Now $G$ acts on $Y$
...
it, using the fact that the \kahler quotient is (modulo issues of stability)
...
$U(1) \subset SU(2)_R$ action.
Then we have the moment maps
\begin{align}
  \mu_{1} &= \half (\abs{w}^2 - \abs{z}^2) + c, \\
  M_1 = \mu_{2} + \I \mu_{3} &= -\I w z + \alpha,
\end{align}
for constants $c \in \R$ and $\alpha \in \C$.
Let us first compute $X \hkq U(1)$ directly. First we
want to determine $\vec\mu^{-1}(0)$. Setting $M_1 = 0$
means
...
\begin{example}[Modifying \hk manifolds with $U(1)$ symmetry] \label{exa:modifying-hk-with-circle}
...
If $\mu_X(x) \neq 0$ at the fixed points then $Y$ will be a \hk 
manifold, of the same dimension as $X'$. \fixme{...}

\end{example}

\begin{example}[Multi-Eguchi-Hanson spaces as \hk quotients]
This is a special case of \autoref{exa:modifying-hk-with-circle}.
We begin with the space $X = \bbH^{n+1}$. 
In structure $I_1$, $X$ is naturally identified with
$\C^{2n+2}$ with coordinates $(z_0, w_0, z_1, w_1, \dots, z_{n+1}, w_{n+1})$.
...
$X \to \R^3 \otimes_\R \fu(1)^{n+1} \simeq \R^{3n+3}$, given by
\begin{align}
...
at the positions $x_i = \vec c_i - \vec c_0$.
...
\begin{exercise} Show that $\deg E$ is well defined, independent
of the choice of connection $D$.
\end{exercise}
...
Moreover $\fG$ acts on $\cA$, by pullback.

\begin{prop}[Moment map for $\fG$ action on $\cA$] \label{prop:unitary-connections-moment-map}
The action of $\fG$ on $\cA$ (thought of as a space of 
unitary connections $D$) admits a moment map, given by
\begin{equation}
  \mu(D) = -F_D.
\end{equation}
(More precisely, this means: suppose given an element $Z \in \Lie \fG$,
represented by a smooth section of $\End E$; 
then we have $\mu_Z(D) = -\int_C \Tr(Z F_D)$.)
\end{prop}

\begin{pf} The key facts are the action of $Z \in \Lie \fG$ 
by infinitesimal gauge transformation,
\begin{equation} \label{eq:inf-gauge}
  \rho(Z) = DZ
\end{equation}
and the differential of $\mu_Z$, acting on a tangent vector
$\beta \in \Omega^1(\fu(E)) = T\cA$:
...
$\mu^{-1}(0)$ is the subset of $\cA$
consisting of \ti{flat} connections. This is empty
unless $d=0$. More generally for arbitrary $d$
we can consider connections with central curvature,
the constant determined by the degree:
\begin{equation}
  Y = \mu^{-1}\left(c\right), \qquad c = - 2\pi\frac{d}{K} \omega {\textbf 1}.
\end{equation}
Then we define:

\begin{defn}[Moduli spaces of (semi)stable bundles]
Let $\cA' \subset \cA$ be the subset of \ti{irreducible}
unitary connections. Then
\begin{equation}
  \Bun^s_{K,d}(C) = \cA' \kql{c} \fG, \qquad \Bun_{K,d}(C) = \cA \kql{c} \fG.
\end{equation}
\end{defn}

Note that these are both infinite-dimensional quotients.
Nevertheless they still make sense, at least
as sets --- in fact $\Bun^s_{K,d}(C)$ is even a manifold,
and $\Bun_{K,d}(C)$ is compact, 
...
$\bar\partial_D$ for a unique $[D] \in \Bun_{1,d}(C)$.
\end{thm}

\begin{pf} \fixme{abelian Hodge theory}
\end{pf}

\begin{thm}[Narasimhan-Seshadri theorem]
For any $[D] \in \Bun^s_{K,d}(C)$ the $(0,1)$ part $\bar\partial_D$
induces the structure of \ti{stable} holomorphic vector bundle on $E$.
Conversely, any stable holomorphic structure 
on $E$ is equivalent to $\bar\partial_D$ for a unique $[D] \in \Bun^s_{K,d}(C)$.
\fixme{check!}
\end{thm}
...
First suppose $[D] \in \Bun^s_{K,d}(C)$. Thus $D$
