...
\begin{equation}
 \cZ \to \left\{ uv = \prod_{i=1}^n (\eta - \eta_i) \right\} \subset [(\cO(n) \oplus \cO(2) \oplus \cO(2)) \to \C\PP^1].
\end{equation}
This map is an isomorphism on most of the fibers, but not on 
the fibers over $\zeta$ for which some $\eta_i$ coincide.
In these fibers the map contracts some complex submanifolds.
\fixme{should say more here!}
