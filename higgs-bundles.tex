...
\begin{thm}[Reconstruction of \hk manifold from its twistor space]
\end{thm}
\fixme{...}
...
In structure $I_1$ we have $\C \times \Sigma_\tau$.
In particular, we have a modulus $\tau$.
Let $a = x_2 + \I x_3$.
Away from $\zeta = 0$ and $\zeta = \infty$, can write
\begin{align*}
  \cX_1 &= \exp \left(\frac{a}{\zeta} + \I \theta_1 + \bar{a} \zeta \right),\\
  \cX_2 &= \exp \left(\frac{\tau a}{\zeta} + \I \theta_2 + \bar{\tau} \bar{a} \zeta \right).
\end{align*}
\fixme{Compute all forms and the metric from this.}
\end{example}
