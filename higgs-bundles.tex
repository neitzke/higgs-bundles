...
% Thus $\im \de \mu \subset \fg^*$ is not annihilated by any $Z \in \fg$.
% Now, by the Hahn-Banach theorem, if $\de\mu: TX \to W$ were
% not surjective, then there would be some linear functional
% $\eta \in W^*$ which annihilates $\im \de\mu$.
% \fixme{but this doesn't seem to help}
...
\begin{equation}
  \sum_{i_1, \dots, i_k} {F_{i_1 \cdots i_\ell}} \partial_{i_1} \cdots \partial_{i_\ell}
\end{equation}
...
\begin{equation}
  \sigma = \I^\ell \sum_{i_1, \dots, i_k} {F_{i_1 \cdots i_\ell}} p_{i_1} \cdots p_{i_\ell}
\end{equation}
\end{defn}
...
Suppose $D$ is elliptic. Then for any $k$,
...
\end{equation}
Then we can apply \autoref{prop:banach-symplectic-quotient-is-symplectic}
to see that $\cN_{K,d}(C)$ is symplectic. The proof that it 
is \kahler follows exactly the lines of the finite-dimensional
...
For general $d$ something more is required. \fixme{...}
\end{pf}
...
Moreover, from this description 
...
\begin{exercise} \label{exc:metric-on-jac-C-genus-one}
Write an explicit formula for the \kahler metric on $\Jac(C)$
when $C$ has genus $1$, with complex modulus $\tau$.
\end{exercise}
...
which appeared in \autoref{exc:metric-on-jac-C-genus-one}.)
\end{exercise}
...
question is to construct functors in both directions.)
...
a polygon with edges identified.)
\end{exercise}
...
\begin{example}[Bundles of rank $2$ over a genus $2$ curve] Now let us consider the case where $K=2$ and $C$ has genus $g = 2$. In this case
we have $\dim_\C \cN_{2,d}(C) = 5$. For $d=1$, this space is 
a compact \kahler manifold. \autoref{prop:tensor-line-is-isometry} says
that it has a natural action of the complex 2-dimensional torus
$\Jac C$ by isometries. Dividing out this action we
are left with a compact \kahler manifold of complex dimension $3$.
\fixme{...}
\end{example}
