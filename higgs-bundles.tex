...
\begin{prop}[The $\omega_i$ determine the complex structures]
If $X$ is \hk then
\begin{equation}
...
Now we generalize from $\R^3 \times S^1$ to a \ti{circle bundle}
with connection.
...
\begin{equation}
  \omega_i = \de x_i \wedge \tTheta - V \star \de x_i.
\end{equation}
To check that these are indeed closed,
\begin{equation}
  \de \omega_i = \de x_i \wedge \star \de V - \de V \wedge \star \de x_i = 0
\end{equation}
(the last equality because all of these forms are pulled back
from $\R^3$, and on $\R^3$ we always have $\alpha \wedge \star \beta - \beta \wedge \star \alpha = 0$ for 1-forms $\alpha, \beta$.)
Then define
\begin{equation}
  \Omega_1 = \omega_2 + \I \omega_3 = V \de z_1 \wedge \alpha_1
\end{equation}
where we introduced
\begin{equation}
  z_1 = x_2 + \I x_3, \qquad 
...
\end{equation}
This motivates the definition of an almost complex 
structure $I_1$: it acts
by the eigenvalue $+\I$ on the forms $\de z_1$ and $\alpha_1$.
Similarly we can define $I_2$, $I_3$.
\fixme{obeying algebraic relations}
\fixme{as complex manifold, it has a global holomorphic function
$z_1$ and holomorphic vector field
$V \partial_\chi - \I \tilde \partial_1$, where $\tilde \partial_1 \cdot \tilde\Theta = 0$ ie parallel lift of $\partial_3$ --- would
integrate to $\C^\times$ action --- integrability comes from 
$[\tilde\partial_z, V \partial_\chi - \I \tilde \partial_1] = 0$}

Now we compute $g$ from $I_1$ and $\omega_1$:
\begin{equation}
  \omega_1 = V \im \alpha_1 \wedge \re \alpha_1 - V \re \de z_1 \wedge \im \de z_1
\end{equation}
which gives
\begin{align}
...
respect to \ti{two} of the complex structures $I_{\vec s}$.
Which ones? (Hint: to see that it's complex with respect 
...
rise to $T^* \C\PP^1$, the others are something else.

\begin{example}[Multi-Eguchi-Hanson spaces] \cite{MR520463} More generally we can fix a collection
...
gives an $S^2$ in $X$. This $S^2$ is a complex submanifold with respect to two
of the complex structures $I_{\vec s}$: indeed $\vec s$ is just the direction
of the line segment.
\fixme{special case where they're all lined up in a row, relation to ALE}
\end{example}
