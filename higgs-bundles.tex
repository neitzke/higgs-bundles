...
\begin{defn} Suppose $X$ is a smooth manifold.
...
\begin{exercise} Show that this is equivalent to the usual definition of complex manifold, namely a space
...
\begin{prop} $I$ is integrable if and only if the \ti{Nijenhuis tensor} $N_I \in C^\infty(\wedge^2 TX)$,
\begin{equation}
...
which induces
\begin{equation}
\wedge^* T^*_\C X = \bigoplus_{p,q} \wedge^{p,q} T^* X , \qquad \Omega^*_\C X = \bigoplus_{p,q} \Omega^{p,q}(X)
\end{equation}
...
\begin{equation}
  \omega(v,w) = g(Iv,w).
\end{equation}
There is also a Hermitian metric on $TX$
considered as a complex vector bundle,
given by
\begin{equation}
  h(v,w) = g(v,w) - \I \omega(v,w) = g(v,w) - \I g(Iv,w).
\end{equation}
\end{defn}
