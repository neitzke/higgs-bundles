...
we get an identification \fixme{watch out for factors $\I$ here}
...
\end{equation}
where $\dot{A}_1$, $\dot{A}_2 \in \cH^1(C)$.
...
Note that this happens \ti{despite} the fact that the underlying
...
let us imagine that we just to take all stable vector bundles
and then equip them with arbitrary Higgs fields: in other words
we define $\cM_{K,d}(C)$ to be a vector bundle over 
$\cN_{K,d}(C)$, with fiber $H^0(C, \End E \otimes K_C)$.
What structure would $\cM_{K,d}(C)$ then get?

The holomorphic tangent space to $\cN_{K,d}(C)$ at $E$ is the space
of harmonic forms $\dot{A}_{\bar z} \in \cH^{0,1}_D(\End E)$.
We can similarly identify $H^0(C, \End E \otimes K_C)$ as the space 
of harmonic forms $\varphi = \Phi_z \in \cH^{1,0}_D(\End E)$.
...
\end{equation}
Thus we get in this way $\cM_{K,d}(C) = T^* \cN_{K,d}(C)$.
...
\begin{equation} \label{eq:Omega-1-AC}
  \Omega_1((\dot{A}_{1{\bar z}}, \dot\Phi_{1z}),(\dot{A}_{2\bar{z}}, \dot\Phi_{2z})) = - 2 \I \int \Tr (\dot{A}_{1{\bar z}} \wedge \dot\Phi_{2z} - \dot{A}_{2\bar{z}} \wedge \dot\Phi_{1z}).
\end{equation}
...
Thus formally (ignoring questions of stability) 
the moduli space of Higgs bundles would appear to be
\begin{equation}
  M_1^{-1}(0) / \fG_\C = \cA^\C_1 \kq \fG_\C.
\end{equation}
As we have noted, though, the holomorphic symplectic quotient of 
a \hk space has another interpretation: we can sometimes interpret
it as a \hk quotient by a smaller group. 
...
$\cA^H$ is naturally an affine space over
...
real symplectic form, naturally extending the one which 
we had on $\cA^h$:
\begin{align}
...
The $\fG$ action on $\cA^H$ admits a \hk moment map, given by:
\begin{equation}
  \mu_2 + \I \mu_3 = - 2 \I \bar\partial_E \varphi
\end{equation}
and
\begin{equation}
  \mu_1 = -F_D - [\Phi, \Phi] - 2 \pi \I \frac{d}{K} \bid \omega_C
\end{equation}
\end{prop}

\begin{pf} We have already computed the holomorphic moment map
$M_1 = \mu_2 + \I \mu_3$. All we need to do is check the 
formula for $\mu_1$. We have
\begin{equation}
  \de \mu_1(\dot A, \dot \Phi) = - D \dot{A} - [\dot\Phi,\Phi] - [\Phi,\dot\Phi]
\end{equation}
and
\begin{equation}
  \rho(Z) = (-DZ, [Z,\Phi])
...
\begin{align}
  ...
  &= - \int_C \Tr(Z D\dot A - [Z,\Phi] \wedge \dot \Phi) \\
  &= \fixme{...}
\end{align}
\end{pf}
