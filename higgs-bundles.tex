...
\subsection{Moduli of stable vector bundles}

Fix a rank $K \ge 1$ and a degree $d \in \Z$.
We are going to define a moduli space $\Bun_{K,d}(C)$ parameterizing
equivalence classes of ``stable'' holomorphic bundles over $C$ of rank $K$
and degree $d$. Moreover this space is naturally \kahler (where 
it is smooth).

\begin{defn}[Slope of a vector bundle] The \ti{slope} of a holomorphic vector bundle $E$ over $C$
is
\begin{equation}
...
Now we  $\cA$ of \ti{all} connections on $E$.
