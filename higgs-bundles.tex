...
of $g^\sf$. It is not invariant under continuous translations in 
...
manifold and its \hk structure \ti{do} extend even over $z=0$, since near
...
\end{equation}
We want to take the symplectic quotient $\cA \kql{c} \fG$.
As a technical preliminary we need to understand the extent to 
which $\fG$ acts freely on $\cA$.
...
\end{prop}
\begin{pf} $D^g = g$ if and only if $g$ is constant under the parallel 
transport of $D$. In particular this means that $g$ is determined by
...
Let $\cA' \subset \cA$ be the subset of irreducible
unitary connections.
Now we define:
...
as topological spaces --- in fact $\Bun^s_{K,d}(C)$ is even a manifold
and $\Bun_{K,d}(C)$ is compact, 
though we haven't proved this yet.
The justification for the names of these spaces will be provided
by \autoref{thm:narasimhan-seshadri} below.
