...
Viewed this way, $\cA$ becomes
simply the space of all unitary
connections on $E$.
This is an affine space, modeled
on the infinite-dimensional \ti{real} linear space $\Omega^{1}(\fu(E))$.
The compatibility of these two pictures of $\cA$ 
...
\begin{equation}
  \mu(D) = - F_D + 2 \pi \frac{d}{K} {\textbf 1} \omega .
\end{equation}
More precisely, this means: suppose given an element 
$Z \in \Lie \fG = \Omega^0(\fu(E))$,
then we have 
\begin{equation}
  \mu_Z(D) = -\int_C \Tr(Z F_D) + 2 \pi \frac{d}{K} \int_C \omega \Tr(Z).
\end{equation}
\end{prop}

\begin{pf} The key facts are the action of $Z \in \Lie \fG = \Omega^0(\fu(E))$ 
...
\begin{equation} \label{eq:harmonic-curvature}
  F_D = 2 \pi \frac{d}{K} {\mathbf 1} \omega.
\end{equation}
...
We remark that $\omega$ is an arbitrary positive $2$-form 
on $C$ with total integral $1$.
At least formally we can imagine taking $\omega$ to be more and more 
concentrated at a single point $z_0$, until in the limit we get
$\omega = \delta(z_0)$. In this limit the connections $D$ obeying
...
because $D^2 = F_D = 2 \pi \frac{d}{K} {\mathbf 1} \omega$ \fixme{...} )
