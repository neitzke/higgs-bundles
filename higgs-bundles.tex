...
\begin{align}
  y_1 &= \sqrt{r/\pi} \sin(\theta/2) \cos\left(\varphi + \chi\right), \\
  y_2 &= \sqrt{r/\pi} \sin(\theta/2) \sin\left(\varphi + \chi\right), \\
  y_3 &= \sqrt{r/\pi} \cos(\theta/2) \cos\left(\chi\right), \\
  y_4 &= \sqrt{r/\pi} \cos(\theta/2) \sin\left(\chi\right),
\end{align}
and then compute directly that
\begin{equation}
  g = \sum_{i=1}^4 \de y_i^2.
\end{equation}
\end{example}


\begin{exercise} \label{exc:gibbons-hawking-R4}
Show that the \hk structure on $X$ extends
