...
\begin{align*}
  \cX_A &= \exp \left(\zeta^{-1} \frac{z_1}{2 \sqrt{\im \tau}} - \I \theta_1 + \zeta \frac{\bar{z}_1}{2\sqrt{\im \tau}} \right),\\
  \cX_B &= \exp \left(\zeta^{-1} \frac{\tau z_1}{2\sqrt{\im \tau}} + \I \theta_0 + \zeta \frac{\bar \tau \bar {z}_1}{2\sqrt{\im \tau}} \right),
\end{align*}
where we introduced
\begin{align}
  \theta_0 &= \sqrt{\im \tau} x_0 - \frac{\re \tau}{\sqrt{\im \tau}} x_1, \\
  \theta_1 &= \frac{1}{\sqrt{\im \tau}} x_1.
\end{align}
The point of these coordinates that the $\Z^2$ action
just shifts $\theta_0$ and $\theta_1$ independently by multiples of $2 \pi$;
the $I_1$-holomorphic combination is $w_1 = x_0 + \I x_1 = \theta_0 + \tau \theta_1$.
Then we have again
\begin{equation} \label{eq:hol-symp-R2T2}
  \frac{\de \cX_A \wedge \de \cX_B}{\cX_A \cX_B} = -\I \Omega(\zeta)
\end{equation}
and so again $X \simeq \C^\times \times \C^\times$.
...
\begin{exercise}
Check \eqref{eq:hol-symp-R2T2}. As we have explained,
...
Next let us describe some of the abstract properties of $\cZ$:
...
$X$ carries a canonically defined \hk metric. If $\cZ$ is the twistor space of a
\hk manifold $X'$, then $X' \subset X$, and the \hk structures
agree.
\end{thm}
...
\item The space of \ti{all} holomorphic sections of $\cZ$ provides
a natural \ti{complexification} of $X$. For example, when $X = \bbH$,
$\cZ$ is the total space of $\cO(1) \oplus \cO(1) \to S^2$. 
The space of sections of this bundle is a complex 4-dimensional vector space,
equipped with a real structure; we recover the original $\bbH$
