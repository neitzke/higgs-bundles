...
\begin{lem} Suppose given a lattice $\Gamma$ with an
isomorphism $\Gamma \simeq \Gamma^*$,
homomorphism $Z:\Gamma \to \C$,
and a map (not homomorphism) $DT: \Gamma \to \Z$.
Let $T$ be the torus
\begin{equation}
  T = \Hom(\Gamma, \C^\times).
\end{equation}
...
as follows: $\cX^\sf(\zeta): T \to T$ is translation by
...
has a finite limit.
\item $\cX$ is analytic away from a countable collection of rays 
...
\fixme{be precise about clockwise/counterclockwise; and check sign}
\begin{equation}
  \cX \to \cX \exp \left[ (n\mu)^* \sum_{n \ge 1} DT(n\mu) \log(1 + \cX_{n\mu}) \right].
\end{equation}
or more concretely,
\begin{equation}
  \cX_\gamma \to \cX_\gamma \prod \fixme{...}
\end{equation}
\end{itemize}
\end{lem}

\begin{pf} Suppose there were two such $\cX$. \fixme{..} 
\end{pf}
