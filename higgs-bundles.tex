...
\end{center}
I thank the participants of the spring 2016 course at UT Austin
for many corrections and improvements.
}
...
$G$ acts on $X$, with \hk moment map $\vec\mu$.
...
compact group $G$ acts on $X$, with \hk moment map $\vec\mu$.
...
\begin{defn}[Norm of a linear map] Say $V$ and $W$ are Banach
...
\begin{thm}[Open mapping theorem for Banach spaces] Suppose $T: V \to W$ is a 
...
\begin{example}[Spaces of bounded linear maps] If $V$ and $W$ are Banach
...
\begin{thm}[Hahn-Banach theorem] If $V$ is a Banach space, $W \subset V$
...
% $v \mapsto \iota_v \omega$ is a bijection.
...
acting properly and freely,
and the tangent spaces to orbits,
$T_x (Gx) \subset T_x X$, are closed and complemented.
Then $X / G$ is a manifold and $TX / \rho(\fg) \simeq T(X/G)$,
as in \autoref
{prop:free-proper-quotients-are-manifolds}.
\fixme{ref Bourbaki}
\end{prop}
...
sheaf kernel of bundle map is a bundle}

\begin{lem}[Saturation of subsheaves of bundles over curves] \label{lem:saturation}
...
(The image sheaf of a map $E \to E'$ 
is a subsheaf of the locally free sheaf of sections of $E'$, so
it is itself a locally free sheaf,
and thus it is the space of sections of some bundle, but nevertheless it is not
...
$\det \gamma = 0$.
Thus the sheaf $\ker \gamma$ is a nontrivial proper
...
Thus $\gamma$ induces a map $\rho$ fitting into a diagram of holomorphic vector bundles,
\begin{equation}
\begin{tikzcd}
0 \arrow[r] & K \arrow[r] & (E, \bar{\partial}_D) \arrow[r] \arrow[d,"\gamma"] & G \arrow[d,"\rho"] \arrow[r] & 0 \\
0 & H \arrow[l] & (E, {\bar\partial_{D_\infty}}) \arrow[l] & I \arrow[l] & 0 \arrow[l]
\end{tikzcd}
\end{equation}
with $\rank G = \rank I$ and $\det \rho \neq 0$.
The existence of $\rho$ implies
\begin{equation} \label{eq:degGlF}
  \deg G \le \deg I.
\end{equation}
Moreover, since $(E,\bar\partial_D)$ was assumed stable,
we must have $\deg K < 0$, so $\deg G > 0$, and thus $\deg I > 0$.
...
The bottom row leads to a bound on all
connections $D'$ in the orbit of $(E,\bar\partial_{D_\infty})$,
as follows.
First, note that because $(E,\bar\partial_{D_\infty})$ is unstable
we cannot make $J(D') = 0$; indeed the unstable
...
of a connection $D'' \in \cO$ such that
...
\fixme{comment on other approaches, Yang-Mills flow}
...
\begin{defn}[Doubled connection] A \ti{doubled connection} on $(E,h)$ is a pair
...
\begin{defn}[Harmonic doubled connection]
...
\begin{defn}[Irreducible doubled connections] A doubled 
...
we have:
\begin{prop}[Gauge group acts almost freely on irreducible doubled connections]
...
\begin{defn}[Moduli space of harmonic doubled connections]
\begin{equation}
...
\begin{lem}[Almost-vanishing for the doubled gauge complex] If $(D,\Phi) \in \cA^{H,s}$, then $\coker \hat{D}$ is $4$-dimensional,
...
\begin{thm}[Polystable Higgs bundles admit harmonic doubled connections] \label{thm:polystable-higgs-bundles-harmonic}
We have:
\begin{itemize}
...
method.
% Suppose we have a sequence $(D_n, \Phi_n)$ of harmonic doubled connections,
