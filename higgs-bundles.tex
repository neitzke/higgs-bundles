...
\begin{example}[Semiflat metric in one dimension] \label{exa:semiflat-1d}
...
define $\Omega(\zeta) \in \Omega^2(X)$ by
\begin{equation}
  \frac{\de \cX_A \wedge \de \cX_B}{\cX_A \cX_B} = -\I \Omega(\zeta).
\end{equation}
\end{example}

\begin{exercise} Show that there is a \hk structure on the 
space $X$ in \autoref{exa:semiflat-1d}, with holomorphic symplectic
form $\Omega(\zeta)$. (One way to do this --- maybe not
the simplest --- is to identify
$X$ as a quotient of a Gibbons-Hawking space, 
with base $U = \cB \times \R$, and $V = \im \tau$.)
What is the complex manifold $(X,I_1)$?
\end{exercise}
