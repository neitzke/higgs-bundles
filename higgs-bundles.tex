...
of the shape $\begin{pmatrix} 0 & 1 \\ -1 & 0 \end{pmatrix}$). 
...
\end{equation}
and cyclic permutations. (Here similarly we view $g$ as a map $TX \to T^*X$,
...
\begin{equation}
  \omega_i = \de x_i \wedge (\Theta/2\pi) + V(\vec x) \star_{\R^3} \de x_i.
\end{equation}
\fixme{should include direct proof that these forms really make
up a hyperkahler structure}
...
\begin{example}[Multi-Eguchi-Hanson spaces] More generally we can fix a collection
