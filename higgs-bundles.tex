...
\eqref{eq:eta-GH}. Then $\cZ$ is parameterized by the $\cO(2)$-valued
...
The holomorphic symplectic form is
\begin{equation}
  \Omega = \frac{\de U \wedge \de V}{n \frac{\partial}{\partial \eta} \prod (\eta - \eta_i)}.
\end{equation}
(Note that then $n=1$ we just have $UV = \eta$, i.e. $\eta$ can be eliminated
...
(Hint: one way to do this is to construct in some concrete way 
the functions $U$, $V$.)
\end{exercise}
