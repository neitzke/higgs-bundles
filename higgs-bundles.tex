...
\begin{equation}
  J^2 = \int_C \left[\nu\left(\frac{\star F}{2 \pi \I}\right)\right]^2
\end{equation}
where we use the norm on matrices given by
$\nu(M) = \sqrt{\Tr M^*M} = \sum \abs{\lambda_i}$.

Suppose we work formally for a moment, avoiding questions of
smoothness etc.
And suppose that $\cO$ contains a minimum of $J$.
Then we consider an infinitesimal gauge transformation by 
$Z \in \Lie \fG_\C = \Omega^0(\End E)$.
The infinitesimal change of $\star F_D$ is given by $\I D^* D Z$.
Thus if we choose $Z$ obeying
\begin{equation}
  \I D^* D Z = -\star F_D
\end{equation}
the gauge transformation will reduce $J$, unless 
already $F_D = 0$.
...
$\norm{F_{D_n}}$ approaches its infimum. 
Using the Uhlenbeck compactness
theorem \autoref{thm:uhlenbeck-weak} we can make gauge transformations
such that this sequence becomes \ti{convergent}, to 
some limiting connection $D_*$. If 
$D_* \in \cO$ then we are done. If not, then we \ti{use}
$D_*$ to prove that the original holomorphic bundle 
$(E,\bar\partial_D)$ is 
actually unstable. This is the trickiest part.

If we assume that $D_* \not\in \cO$ then there 
...
show that this kernel cannot disappear in the limit,
...
The top row of the diagram implies the existence
...
On the other hand the bottom row leads to a bound on \ti{all} 
connections in the orbit of $(E,\bar\partial_{D^*})$,
\begin{equation} \label{eq:J-lower-bound}
  J \ge 2 \deg F.
\end{equation}
This is easier: we get it from the shape of the 
...
For this, we fix some element $\tilde{d} \in \cN_{1,d}(C)$ (i.e. 
a holomorphic / harmonic line bundle.) 
If $d=0$ then it would be natural to take
$\tilde{d}$ to be the trivial bundle. 
Then we fix a bundle $E$ together with an isomorphism
$\det E \simeq \tilde{d}$. Then we construct a moduli 
space $\cS\cN_{K,\tilde d}(C)$ by repeating all our previous 
constructions, now with the extra condition that the connections
$D$ which we consider restrict to the harmonic connection 
on $\det E$. \fixme{...}

\begin{example}[Bundles of rank $2$ with fixed determinant 
over a genus $2$ curve] Now let us consider the case where $K=2$ and $C$ has genus $g = 2$. In this case we have $\dim_\C S\cN_{2,d}(C) = 3$. When $d=1$ this is in fact
a compact \kahler manifold; let's consider that case first.
\fixme{...}
...
\begin{example}[Hitchin section] \fixme{...}
\end{example}
...
One thing we could try is just to take a vector bundle over 
$\cN_{K,d}(C)$, with fiber $H^0(C, \End E \otimes K_C)$.
The holomorphic tangent space to $\cN_{K,d}(C)$ is the space
of harmonic $(0,1)$-forms valued in $\End(E)$,
which is naturally dual to $H^0(C, \End E \otimes K_C)$.
Thus the resulting space would be $T^* \cN_{K,d}(C)$.
In particular this is a \ti{holomorphic symplectic} manifold.

What we will actually do is a bit better. 
As before, we fix a bundle $E$ of degree $d$.

We define a ``doubled'' version of the space of connections,
\begin{equation}
  \cA^H = \cA^{\bar\partial} \times \Omega^{1,0}(\End E).
\end{equation}
The space $\cA^H$ carries a natural holomorphic symplectic form
\begin{equation}
  \Omega_1 = \fixme{...}
\end{equation}

Formally the moduli space of Higgs bundles would appear to be
\begin{equation}
  \cA^H = M^{-1}(0) / \fG_\C.
\end{equation}
\fixme{...}
