...
$$D: L^2_k(E) \to L^2_{k-\ell}(E),$$
and we have:
\begin{itemize}
\item $D$ is \ti{Fredholm}, i.e. it has closed range, 
and finite-dimensional 
kernel and cokernel.
\item $\ker D$ and $\coker D$ contain only
...
In particular, using this description we see that $\Jac(C)$
is \ti{independent} of the chosen metric on $C$.
Moreover, from this description 
we get an identification $T_x \Jac(C) \simeq H^1(C,\R)$.
...
on $\Jac(C)$.
Our realization of $T_x \Jac(C)$ via the symplectic quotient
identifies it as $\ker (\de \oplus \de^*)$,
i.e. as the space of harmonic forms $\cH^1(C)$.
The relation to the description of $T_x \Jac(C)$
above is just that in each cohomology class
we choose the unique harmonic representative.
Now, as usuual for symplectic quotients, 
...
$\cN_{K,d}(C)$ is compact. \fixme{use Uhlenbeck compactness}
\end{thm}
