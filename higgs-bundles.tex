...
{These are the notes for a \ti{currently ongoing} (Spring 2016)
course at UT Austin. They are
extremely incomplete, unreliable, full of mistakes and omissions, 
and being updated frequently. The latest PDF can always be found
at
\begin{center}
...
\begin{exercise} Suppose $X$ is a manifold
...
\begin{thm}[de Rham theorem for compact manifolds]
...
only if $\zeta = \pm \I \frac{z}{\abs{z}}$. 
...
But this tameness seems to be essentially equivalent to
...
a vector space $V$ over $\R$ or $\C$, equipped with a 
topology in which $V$ is complete, and such that there
exists a norm on $V$ inducing the topology.
\end{defn}

Whenever necessary we will \ti{choose} a particular norm inducing
the topology. One has to check that all of our statements
which use a norm are actually independent of the choice of norm.
...
Some authors would call this a ``weakly nondegerate'' pairing.
...
Then $f$ admits a smooth inverse on an open neighborhood of
$f(v) \in W$.
\end{thm}
...
Then write
\begin{equation}
  \norm{s}^2_{p,k} = \int_X \left(\norm{s}^2 + \norm{\nabla s}^2 + \norm {\nabla \nabla s}^2 +
    \cdots + \norm{\nabla^k s}^2\right) \de vol_X.
\end{equation}
...
Banach space $L^p_k(E)$ actually does not depend on this choice.
\end{example}
...
we needn't worry too much about this.) \fixme{right?}
...
\begin{defn}[Moment maps and symplectic quotients] \label{def:banach-moment-map}
...
\begin{thm}[Symplectic quotient is symplectic]
Suppose $X$ is a symplectic Banach manifold, with a group $G$
acting properly on it, with moment map $\mu$. 
Suppose that $G$ acts freely on $\mu^{-1}(0)$, the tangent
spaces $T = T_x (Gx)$ to $G$-orbits are complemented
by $T^{\perp_\omega}$, and
\begin{equation}
...
\begin{equation}
  \pi^* \omega_{X \kq G} = \iota^* \omega.
\end{equation}
\fixme{need to check the conditions on $G$ action are 
really enough to get $\mu^{-1}(0)$ to be a manifold; idea:
they mean $\mu$ maps into some smaller space
than $\fg^*$, and $\de \mu$ is surjective there}
\end{thm}
...
ellipticity. Something much nicer can be found in \fixme{Wells}.

Suppose $X$ is a compact manifold carrying
...
We could try to ``solve'' \eqref{eq:Def}
...
\end{equation}
This looks like it has some chance of working, at least if
the behavior of $\hat f(p)$ near $p=0$ is somehow controlled.
That problem has to do with the \ti{low frequency} modes of $f$, and 
...
use later on.
These are basically based on the special case of the operator
\begin{equation}
  \de + \de^*: \Omega^1(C) \to \Omega^2(C) \oplus \Omega^0(C)
\end{equation}
on a Riemannian 2-manifold $C$.
In local coordinates around $x = 0$, orthonormal at $0$, 
this operator could be written as
\begin{equation}
 f_1 \de x_2 + f_2 \de x_2 \mapsto (\partial_2 f_1 - \partial_1 f_2) \de x_1 \wedge \de x_2 + (\partial_1 f_1 + \partial_2 f_2)
\end{equation}
or relative to the obvious bases,
\begin{equation}
  \begin{pmatrix} \partial_2 & \partial_1 \\ -\partial_1 & \partial_2 \end{pmatrix}
\end{equation}
Thus the symbol is
\begin{equation}
  \begin{pmatrix} p_2 & p_1 \\ -p_1 & p_2 \end{pmatrix}
\end{equation}
...
We are going to define a moduli space $\cN_{K,d}(C)$ parameterizing
equivalence classes of holomorphic bundles over $C$ of 
...
of $L^2_k$ connections. $\cA^h_k$ is a Banach manifold,
...
on $\cA_k$ are compatible, i.e. $\omega(I \alpha, I \beta) = \omega(\alpha,\beta)$.
\end{exercise}
Thus $\cA_k$ is \kahler.
Moreover, it carries a natural action of $\fG_{k+1}$:
...
This action preserves the \kahler structure.
...
$\beta \in \Omega^1_{k}(\fu(E)) = T\cA^h_k$:
...
Fortunately the corresponding operator
\begin{equation}
 \hat{D} = D \oplus D^*: \Omega^1(\fu(E)) \to \Omega^2(\fu(E)) \oplus \Omega^0(\fu(E)).
\end{equation}
is elliptic, just as for the example of $\de + \de^*$ discussed
above. This has the following encouraging
consequences:

\begin{prop}[Elliptic regularity for the gauge complex]
Define
\begin{equation}
 \hat{D} = D \oplus D^*: \Omega^1_k(\fu(E)) \to \Omega^2_{k-1}(\fu(E)) \oplus \Omega^0_{k-1}(\fu(E)).
\end{equation}
Then $\hat{D}$ is \ti{Fredholm}, i.e. it has closed range, 
and finite-dimensional 
kernel and cokernel.
Moreover, $\ker \hat{D}$ and $\coker \hat{D}$ contain only
smooth sections (so in particular they are independent of $k$.)
Finally $\coker \hat{D} = \ker \hat{D}^*$.
\end{prop}
...
If $D \in \cA^s_k$, then
$\coker \hat{D}$ is $2$-dimensional.
\end{lem}
(Informally: both $H^0$ and $H^2$ of the complex \eqref{eq:gauge-complex} are
$1$-dimensional.)
\begin{pf}
We represent
$\coker \hat{D}$ as $\ker \hat{D}^*$, where
\begin{equation}
  \hat{D}^* = D \oplus D^*: \Omega^0_k(\fu(E)) \oplus \Omega^2_k(\fu(E)) \to \Omega^1_{k-1}(\fu(E)).
\end{equation}
...
otherwise, $H^2$ is just spanned by $\star (\bid)$.
\end{pf}

Morally what we used here is just the statement that 
$\fG$ acts freely on $\cA^s_k$.
...
Banach space inverse function theorem (applied to a quotient
by the 1-dimensional subspace spanned by $\star \bid$.)
\fixme{...}
\end{pf}
...
\end{thm}
\begin{pf} This can be obtained as an application of 
the Atiyah-Singer Index Theorem, which gives a formula 
for the index 
\begin{equation}
  \ind \hat{D} = \dim \ker \hat{D} - \dim \coker \hat{D}
\end{equation}
when $\hat{D}$ is \ti{any} elliptic
differential operator over a compact manifold. 
The formula is complicated, but all we need to know is that 
it does not depend on all details
of $\hat{D}$ --- it depends only on the symbol, i.e.
the highest-degree terms of the differential operator.

For the operator $\hat{D}$ we have here, the symbol
...
complex, whose index is known: it is $-\chi(C) = 2g-2$.
Thus the index formula gives
\begin{equation}
  \ind \hat{D} = (2g-2) K^2.
\end{equation}
...
connection is to have $K=1$, in which case we get dimension
...
However, one does not quite get a \ti{canonical} isomorphism.
...
\autoref{prop:tensor-line-is-isometry} all degrees are
...
\begin{equation}
  \Jac(C) \to H^1(C, U(1)).
\end{equation}
In fact this map is an isomorphism. To see this, it's enough
to show that, if $a \in \Omega^1(C)$ and $\oint_\wp a = 0$
for all paths $\wp$ in $C$, then $a$ is exact; but that is just 
the Poincare lemma.

Thus $\Jac(C)$ is the compact torus $H^1(C,U(1))$. In particular,
topologically (even symplectically) it does not depend on
the complex structure of $C$.
\end{example}

\begin{thm}[Compactness of $\cN_{K,d}(C)$] \label{thm:Ns-is-compact}
$\cN_{K,d}(C)$ is compact. \fixme{...}
\end{thm}
...
\begin{example}[Stable bundles of rank $2$ over a genus $2$ curve] Now let us consider the case where $K=2$ and $C$ has genus $2$. \fixme{...}
\end{example}
...
\subsection{Holomorphic description}

\fixme{material below needs to be organized}
...
exists a unique Hermitian metric $h$ on $\cL$ such that the Chern
connection $D_h$ is Einstein.
\end{lem}
\begin{pf}
...
We can write $h' = e^f h$ for some real-valued function $f$ on $C$.
Then we have
\begin{equation}
  F_{D_{h'}} = F_{D_h} + \partial \bar\partial f.
\end{equation}
...
This gives the desired $f$.
\end{pf}

There is another way of thinking about this result, which
unfortunately will bring up a slightly confusing point.
Namely, suppose we consider a fixed line bundle $E$, with a
fixed Hermitian metric. Then the analogue of ``changing the 
Hermitian metric'' is ``making a gauge transformation
valued in $\fG_\C$ but not in $\fG$.'' \fixme{...}
...
% $\bar\partial_D$ for a unique $[D] \in \cN_{1,d}(C)$.
% \end{thm}
...
For any $[D] \in \cN^s_{K,d}(C)$ the $(0,1)$ part $\bar\partial_D$
