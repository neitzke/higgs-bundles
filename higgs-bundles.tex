...
We are going to define a moduli space $\cN_{K,d}(C)$ which can be 
thought of in two ways:
\begin{itemize}
\item It is a moduli space of unitary connections on 
...
\ti{symplectic}.
\item It is a moduli space of holomorphic vector 
...
\end{equation}
\end{exercise}
Moreover, $\cA_k$ it carries a natural action of $\fG_{k+1}$:
...
vanishes because $E'$ is preserved by $\bar\partial_D$). Then we compute
\begin{equation}
...
%   \I D^* D Z = \star F_D.
% \end{equation}
The resulting first-order change in $F_D$ is $F_D Z$,
so the change in $\norm{F_D}^2$ is
\begin{equation}
 2 \int_C \Tr (F_D Z \wedge \star F_D)
\end{equation}
If we are at the minimum then this must vanish for all $Z$,
but this can happen only if $F_D = 0$.
So all we need to do is show that a
...
theorem \fixme{...} we can make gauge transformations
such that this sequence becomes \ti{convergent}, so 
some limiting connection $D^*$. If 
$D^* \in \cO$ then we are done. If not, then we \ti{use}
$D^*$ to prove that the original holomorphic bundle 
$(E,\bar\partial_D)$ is 
actually unstable. This is the trickiest part.
First we show that at least there is a 
holomorphic map $(E,\bar\partial_D) \to (E,\bar\partial_{D^*})$
(use the fact that such maps do exist if $D^*$ is replaced
by $D_n$, plus some analysis which morally says a zero mode 
which exists for all the $D_n$ cannot \ti{disappear} in the limit.)
But the orbit 
...
we have $\dim_\C \cN_{2,d}(C) = 5$. When $d=1$ this is in fact
