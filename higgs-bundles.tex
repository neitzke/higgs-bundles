...
\begin{itemize}
\item $\cM$ is a partial compactification of $T^* \Bun(C,G)$, where $\Bun(C,G)$ is the moduli space of stable $G$-bundles on $C$; 
from this point of view Lagrangian submanifolds are related to \ti{D-modules} on $\Bun(C,G)$, basic objects on ``$A$ side'' 
of the geometric Langlands correspondence, \insfig{higgs-bundles-2}{0.55}
...
the \ti{\hk} structure. This says in short that $\cM$ has many
...
This structure is rather rigid and gives a lot of constraints, e.g.
it implies that $\cM$ carries a Ricci-flat metric, and even lets 
