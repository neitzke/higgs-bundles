...
\begin{equation}
  \cN^s_{K,d}(C) = \cA^s \kq \fG, \qquad \cN_{K,d}(C) = \cA \kq \fG.
\end{equation}
\end{defn}
...
\begin{thm}[$\cN^s_{K,d}(C)$ is a \kahler manifold] \label{thm:N-is-kahler}
...
\begin{thm}[Dimension of $\cN^s_{K,d}(C)$]
$\dim_\C \cN^s_{K,d}(C) = (g-1)K^2+1$ where $g$ is the genus of $C$.
\end{thm}
\begin{pf} \fixme{index theorem}
\end{pf}

\begin{thm}[Compactness of $\cN_{K,d}(C)$]
